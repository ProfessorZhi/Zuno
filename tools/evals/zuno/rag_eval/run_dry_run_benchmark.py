...
DRY_RUN_OUTPUT_DIR = REPO_ROOT / "docs" / "evidence" / "goal05-phase22-dry-run-output"
...
def run_dry_run() -> dict[str, Any]:
    ...
    for c in candidates:
        case_in = BenchmarkCaseInput(
            ...
            gold_document_refs=tuple(c["gold_document_refs"]),
            gold_evidence_refs=tuple(c["gold_evidence_refs"]),
    ...
    manifest = {
        "benchmark_type": "no_paid_model_dry_run",
        "measurement_state": "BLOCKED",
        "blocked_reason": "blocked_pending_human_review_and_credentials",
        "reviewer_approved_count": 0,
        "benchmark_eligible_count": 0,
        "candidate_cases_evaluated": len(candidates),
        "case_set_hash": case_set_hash,
        "profiles_executed": list(runners.keys()),
    ...
    (DRY_RUN_OUTPUT_DIR / "dry_run_manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    (DRY_RUN_OUTPUT_DIR / "dry_run_results.json").write_text(json.dumps(results_by_profile, ensure_ascii=False, indent=2), encoding="utf-8")

    report = {
        "status": "dry_run_completed_blocked_as_expected",
        "output_dir": str(DRY_RUN_OUTPUT_DIR.relative_to(REPO_ROOT)),
        "candidates_count": len(candidates),
        "measurement_state": "BLOCKED",
        "blocked_reason": "blocked_not_measured",
    }
    (DRY_RUN_OUTPUT_DIR / "dry_run_report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    return report
