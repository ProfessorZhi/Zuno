...
def _write_review_md(
    ...
    lines.append("## Notes\n")
    lines.append("- HotpotQA records use the upstream `_id`/`id` when available; MultiHop-RAG "
                 "and GraphRAG-Bench retain deterministic adapter IDs plus an upstream line ref "
                 "where the source does not publish a record ID.")
    lines.append("- MultiHop-RAG selection requires a nonempty upstream `evidence_list`; the fixed "
                 "slice therefore contains only evidence-complete records.")
    lines.append("- GraphRAG-Bench selection requires an exact case-folded question match in the "
                 "cached official textbook corpus; evidence refs carry the source file and line span.")
    lines.append(f"- The candidate pack has {report['evidence_complete_count']} evidence-complete "
                 f"cases and {report['incomplete_count']} incomplete cases.")
    lines.append("- Provenance remains a compact string for backward schema compatibility; the "
                 "selection rule and GraphRAG corpus hashes are recorded in `source_manifest.json`.")
