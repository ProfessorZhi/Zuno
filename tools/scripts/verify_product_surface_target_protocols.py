...
def verify() -> list[str]:
    ...
    for text in ["01-product-surface.md", "单一完整 Target 架构", "verify_product_surface_target_protocols.py"]:
        if text not in docs_index:
            errors.append(f"docs/history/superseded-document-taxonomy/project-modules/README.md missing Product route: {text}")
    for text in ["docs/history/superseded-document-taxonomy/project-modules/01-product-surface.md", "verify_product_surface_target_protocols.py"]:
        if text not in docs_map:
            errors.append(f".agent/references/docs-map.md missing Product route: {text}")
    for text in ["docs/history/superseded-document-taxonomy/project-modules/01-product-surface.md", "AvailableAction", "Product Projection"]:
