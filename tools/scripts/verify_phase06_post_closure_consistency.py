...
PHASE_FILE = PROGRAM_ROOT / "PHASE06_observability-minimum-black-box.md"
EVIDENCE = REPO_ROOT / "docs" / "evidence" / "phase06-observability-persistence.md"
CLOSURE = REPO_ROOT / "docs" / "evidence" / "phase06-coordinator-closure.md"
PRE_CLOSURE = REPO_ROOT / "docs" / "evidence" / "phase06-pre-closure.md"
READINESS = WORK_PRODUCTS / "phase06-readiness.yaml"
...
def verify_phase06_post_closure_consistency() -> list[str]:
    errors: list[str] = []
    ...
    for label, path, phrases in [
        ...
        ("closure decision", CLOSURE, ["status: approved", "coordinator_approval: approved", "phase06_state: completed"]),
        ("readiness", READINESS, ["current_phase_status: completed", "coordinator_approval: approved", "target_not_current: 0"]),
        ("manifest", MANIFEST, ["current_phase: PHASE08", "id: PHASE06", "state: completed", "id: PHASE07", "state: completed", "id: PHASE08", "state: in_progress", "id: PHASE09", "state: planned"]),
        ("current", CURRENT, ["current_phase: PHASE08", "PHASE06 completed", "PHASE08 in_progress", "PHASE09 planned"]),
        ("closure checklist", CHECKLIST, ["[x] PHASE06 Observability Minimum Black Box"]),
    ]:
        text = _read(path)
...
def main() -> int:
    errors = verify_phase06_post_closure_consistency()
    if errors:
        for error in errors:
            print(f"ERROR: {error}")
        print("PHASE06 post-closure consistency gate failed.")
        return 1
    print("PHASE06 post-closure consistency gate passed.")
    return 0
