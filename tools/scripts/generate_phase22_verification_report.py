...
def build_phase22_verification_report() -> str:
    ...
    return "\n".join(
        [
            ...
            "- 通过：全量 pytest collection 已恢复，收集 `2750 tests`；本轮 Phase22 Eval 回归为 `223 passed, 30 subtests passed`，Repo 文档/契约回归为 `49 passed`，Backend semantic legacy 全套为 `43 passed`，Workspace Phase22 repair 为 `15 passed`，feature-flag runtime cutover 为 `44 passed`，final legacy audit 全套为 `23 passed`，API 知识契约与 Workspace task runtime 定向回归为 `31 passed`。完整 pytest、`-k phase22`、workspace runtime 全文件运行、`tests/repo` 的 `656 tests` 全量运行以及 `tests/api` 的 `167 tests` 全量运行均未在执行上限内产生全量汇总，不能宣称全量通过。",
