...
def build_phase22_closure_summary() -> str:
    ...
    lines = [
        ...
        f"- raw candidate pack integrity_status: {integrity.get('overall_status')}",
        f"- reviewed pack overall_status: {reviewed_summary.get('overall_status')}",
        f"- reviewed pack measurement_state: {reviewed_summary.get('measurement_state')}",
        f"- reviewed pack reviewer_approved_count: {reviewed_summary.get('reviewer_approved_count')}",
        f"- reviewed pack benchmark_eligible_count: {reviewed_summary.get('benchmark_eligible_count')}",
        f"- reviewed pack rejected_or_incomplete_count: {reviewed_summary.get('rejected_or_incomplete_count')}",
        "",
        "## Remaining Blockers",
        "",
