...
FORMAL_INDEX = REPO_ROOT / "docs/history/superseded-document-taxonomy/project-modules/README.md"
...
def verify() -> list[Finding]:
    ...
    if "[`11-infrastructure.md`](./11-infrastructure.md)" not in formal_index:
        findings.append(Finding("INFRA_FORMAL_INDEX_ROUTE", "docs/history/superseded-document-taxonomy/project-modules/README.md does not route the single Infrastructure document"))

    for retired_name in [
        "11-infrastructure-data-services.md",
        "11-infrastructure-consistency-lifecycle.md",
    ]:
        for index_content, label in [(formal_index, "formal index")]:
