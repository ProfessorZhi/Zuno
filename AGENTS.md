...
目标产品口径固定为 AgentChat 驱动的企业知识库 Agentic GraphRAG Workspace。用户在聊天里提出目标，并在勾选知识库时选择标准检索 / 深度检索；GraphRAG、BM25、vector、re-query、rerank 和工具调用由 Single Controller Agent 内部自动规划。不要把 Zuno 写成用户手动选择 RAG / GraphRAG 模式的工具箱，也不要把近期主线写成产品级多 Agent runtime。
