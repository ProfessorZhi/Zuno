...
docs/
  ...
  architecture/     总体 Target 架构正文与架构图展示配对
...
`docs/README.md` 是项目知识入口：`facts/` 回答今天理解 Zuno 必须相信什么，`architecture/` 回答跨层为什么这样设计，`modules/` 在边界稳定后回答责任域如何工作，`history/` 保存不再决定当前设计的过去材料。事实不确定时必须保留 `UNKNOWN`，不能用 Target Architecture 代替 Current 事实。
...
- `docs/architecture/architecture.md`：跨 Product、Domain、Logical Capability、Physical Service/Deployment 的总架构，说明 Agent 闭环、Contract、状态、失败语义和验收。
...
架构文档的统一信息架构和写作规则见 `docs/governance/architecture-document-writing-standard.md`。它是文档治理规范，不是新的架构事实源；总体架构由 `docs/architecture/` 持有，项目事实与状态统一由 `docs/facts/` 持有。
...
阅读路由：用户问“为什么这样设计”时优先读总体架构和对应模块（若已冻结）；用户要求实现、修改或验证时，必须继续读取 `facts/`、Owner 文档、ADR/共享 Contract、Evidence 和当前 Program。每份专题只维护自己的 Canonical Question，不创建跨文档重复状态机。
...
Agent Runtime 任务必须读取 `docs/architecture/architecture.md`、`docs/facts/`、相关 ADR、`docs/evidence/` 和代码；归档专题只用于历史对照。
...
- `src/backend/zuno/knowledge/**` → `docs/architecture/architecture.md`、facts、evidence。
- `src/backend/zuno/memory/**` → `docs/architecture/architecture.md` 与 facts boundary。
- `src/backend/zuno/capability/**` → `docs/architecture/architecture.md`、facts、security governance。
