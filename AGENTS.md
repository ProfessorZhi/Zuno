...
Zuno 的架构文档由 `docs/architecture/architecture.md`、`docs/architecture/architecture-views.md`、`docs/architecture/production-readiness.md`、稳定专题文档、`docs/architecture/architecture.html`、`.agent/architecture/architecture.md`、`.agent/architecture/architecture.html` 和 `.agent/references/` 共同治理。

- `docs/architecture/` 是人类可读的正式架构说明，也是 public architecture source。
- `docs/architecture/architecture.md` 是正式**重文字目标总架构文档**，用自然语言说清“轻量实现、成熟设计”、十一逻辑模块、完整 Agent 闭环、企业知识库场景、Input、Memory、Planning、Capability/Skill/MCP、Tool Runtime、Agentic GraphRAG、Security、Observability、Infrastructure、contract、失败语义和验收；只保留少量关键 Mermaid 图辅助理解。
- `docs/architecture/architecture-views.md` 是十类 canonical views 的唯一 Mermaid 规范图源；每类必须包含一张 Overall 图和至少两张 Local 图，总数不少于三十张。
...
- 稳定专题文档包括 `agent-core-runtime.md`、`memory-and-context.md`、`capability-and-skill-layer.md`、`agentic-retrieval-planner.md`、`eval-observability-and-cost.md`、`input-layer-and-document-processing.md` 和 `knowledge-space-product-configuration.md`；它们只承载稳定专题架构、Current / Target 边界和证据路径，不承载 phase log、roadmap 或 Agent 操作规则。
- `docs/` 应少而精，保留相对稳定、面向人的正式结论；不要把高频变化的执行细节、workflow change log 或 Agent 操作规则塞进 `docs/`。
- `docs/architecture/architecture.html` 是正式架构 HTML 展示页，不是完整设计事实源；它必须读取 `docs/architecture/architecture-views.md` 并使用原生 Mermaid 渲染。
- `.agent/architecture/architecture.md` 是 Agent 侧文字总架构镜像，必须与 `docs/architecture/architecture.md` 完全一致。
- `.agent/architecture/architecture.html` 是 Agent 侧 HTML shell 镜像，必须与 `docs/architecture/architecture.html` 完全一致，并读取同一 `architecture-views.md` 图源。
...
4. `docs/architecture/production-readiness.md`
5. `docs/architecture/document-ingestion-foundation.md`
6. 稳定专题文档
7. `.agent/architecture/architecture.md`
8. `.agent/architecture/architecture.html`
9. `docs/architecture/README.md`
10. `.agent/architecture/README.md`
11. `.agent/references/*`
12. `.agent/templates/*`
13. `.agent/programs/current.md`，有 active program 时还要检查对应 roadmap；no-active 时检查最近归档 program 的 roadmap
14. `README.md` 中的架构摘要
...
总架构 Markdown 内容必须比架构 HTML 更充实：`architecture.md` 负责大段文字设计、模块职责、Current / Target 边界和实施落点；`architecture-views.md` 负责详细 Mermaid 图源；HTML 偏图形展示和全屏查看。设计含义变化先改 `architecture.md`，图形关系变化再改 `architecture-views.md`，最后运行 `python tools/agent/render_architecture.py --write` 同步 `.agent/architecture/architecture.md` 和两份 HTML。禁止把三十张详细图重新堆回 `architecture.md`。
...
历史治理 program 曾按五个成熟系统和八类治理交付物收口；这些完成事实保留在 `docs/history/programs/zuno-eight-deliverables-full-realization-v1/`。当前前台采用 runtime-first 口径；成熟度和 runtime-first 交付物口径以 `docs/architecture/production-readiness.md` 为准。
...
22. `.agent/references/workflow-governance.md`
23. `.agent/references/workflow-update-policy.md`
24. `.agent/references/workflow-maintenance-checklist.md`
...
- 架构替换、目录移动、上下文/记忆、GraphRAG 边界或仓库卫生任务还必须读 `docs/architecture/architecture.md`、`docs/architecture/architecture-views.md` 和 `.agent/architecture/architecture.md`，不得引用已归档旧工作集作为当前入口。
...
当前 `.agent/programs/` 是 no-active 状态。最近完成并归档的 program 是 `zuno-real-unified-runtime-cutover-v1`，归档位置是 `docs/history/programs/zuno-real-unified-runtime-cutover-v1/`。该 program 已完成 Completion / Workspace 产品主路径到 `UnifiedAgentRuntimeService -> compiled LangGraph -> RuntimeDependencyFactory -> Model Gateway / Memory / Corrective Agentic GraphRAG / Tool Control Plane` 的本地 implementation cutover；fixed EnterpriseRAG paired benchmark 仍是 `measurement_blocked`，不得写成 measured pass 或 Agentic GraphRAG superiority 已证明。
...
当前前台采用三层成熟度口径：Current Local Slice 只写已由代码和测试证明的本地闭环；Launchable Prototype Target 是近期可上线企业知识库 Agent 雏形；Production Scale Target 是完整企业级多租户、队列、worker、外部索引、安全治理和运维扩展。成熟度和 runtime-first 交付物口径以 `docs/architecture/production-readiness.md` 为准；Program 3 Mega 已完成 launchable product baseline，本地结论是 `Launchable enterprise Agentic GraphRAG product baseline completed; production scale external deployments remain replaceable targets.` 最近完成的 evidence-span hardening program 已提供 release gate 输出面，但 `Evidence Text Available@5 >= 0.60`、`Source Doc Citation >= 0.85`、`Citation Accuracy >= 0.30`、`Answer Correctness >= standard_rag baseline` 未获得 fixed benchmark measured pass；后续每个 runtime phase 仍只有在真实 API / runtime / UI 路径、focused tests、trace / eval 或 verifier 证明后才能关闭，只写 contract、schema 或 README 不能关闭 runtime phase。
...
最近完成的 `zuno-real-unified-runtime-cutover-v1` 已归档。当前 no-active 状态不能改写 Program 1 / Program 2 / Program 3 Mega 或上一轮 PHASE01-PHASE12 closure evidence；后续仍不得把 Target 伪装成 Current。
...
7. `docs/architecture/architecture-views.md`
8. `docs/architecture/architecture.html`
9. `.agent/architecture/architecture.md`
10. `.agent/architecture/architecture.html`
11. `AGENTS.md`
12. `.agent/references/current-program.md`
13. `.agent/references/docs-map.md`
14. `.agent/references/task-routing.md` 或 `.agent/references/workflow.md`
15. `.agent/programs/`
16. `.agent/system.yaml`、`.agent/references/` skill 文件或 `.agent/templates/`
17. `.gitignore`
18. 如果产生修改，运行验证、提交、推送
...
7. `docs/architecture/architecture.md` 的文字设计与 Current / Target / Future / History 是否仍严格区分。
8. `docs/architecture/architecture-views.md` 是否仍完整维护十类视图和三十张 Mermaid 图。
9. `.agent/architecture/architecture.md` 是否与 `docs/architecture/architecture.md` 完全一致。
10. 两个 `architecture.html` 是否一致，并读取同一个 `architecture-views.md` 图源且可通过渲染校验。
11. verifier / tests 是否覆盖新规则，避免下次漂移。
