...
docs/maintenance/                     Operations、Agent workflow 与历史审查资料
docs/terminology.md                   跨文档术语
.agent/                               Agent 机器路由、规则、Program、模板和验证入口
```
...
8. `.agent/README.md`、`.agent/system.yaml`、`.agent/references/`。需要人类协作流程时再读 `docs/maintenance/agent-workflow/`。
...
Red / Blue 是人工协调的 Architecture Stress Test。它可以盘问 Runtime、Knowledge、Domain、Tool、Security 等边界，但不把工程协作多线程写成 Zuno Runtime 事实。原始记录进入 `docs/maintenance/history/red-blue/`；接受的结果必须写回 Architecture / Module / ADR。
...
更多路由见 `.agent/references/task-routing.md` 和 `.agent/references/workflow.md`。人类可读协作流程见 `docs/maintenance/agent-workflow/README.md`；当前 Program 状态见 `.agent/references/current-program.md`；验证地图见 `.agent/references/verification-map.md`；机器路由见 `.agent/system.yaml`。
