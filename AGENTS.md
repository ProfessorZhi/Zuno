...
历史治理 program 曾按五个成熟系统和八类治理交付物收口；这些完成事实保留在 `docs/history/programs/zuno-eight-deliverables-full-realization-v1/`。当前前台采用 runtime-first 口径：第一版 Current 是 in-process runtime vertical slice 与 Web workspace Agent 产品闭环；production-grade parser、LangGraph persistence、Memory DB、sandbox、external trace/eval 和 Desktop 闭环仍是 Target。
