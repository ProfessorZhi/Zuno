...
本文件是红队会话的运行协议，不是独立的底层思维框架。它执行 `00-charter.md` 的 Claim-driven 选择逻辑，调用 `04` 的攻击工具和 `05` 的问法视角，并把结果路由到 `08` Gap Register；如果本协议与 Kernel 的风险和停止边界冲突，以 Kernel 为准。
...
2. 读取 `project-reconstruction-lab/` 当前事实、Gap 和最近会话；
...
面经数量、方向和深读量以 `project-reconstruction-lab/sources/interview-patterns.md` 的最新刷新快照为准。不得在本协议中复制一组容易过期的固定数字；新增记录不等于已深读，用户自己的真实面试也必须和公开面经分开计权。
...
2. 相关 `docs/<topic>/`、`docs/decisions/`、`docs/status/`、`docs/evidence/`；旧 `modules/` 只作 History 对照；
3. `docs/history/interview-qa/` 中与目标机制相关的既有攻击链；
