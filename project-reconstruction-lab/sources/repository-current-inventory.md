...
- `docs/facts/current-state.md` 为 `NOT_ESTABLISHED`；当前 Eval 文档保持 `MEASUREMENT_BLOCKED`/未证明边界。
...
第二条命令可以定位通用 Document/Claim/Evidence/Agent 结构；第一条命令没有形成法律 Domain Kernel 的可运行 schema、mutation authority、staleness/dependency 和 review/audit 闭环证据。正式状态仍以代码、Migration、测试、Trace、Eval 和 `docs/facts/` 为准。
