...
if ($currentProgram -notmatch "state: active" -or $currentProgram -notmatch "active_program: zuno-unified-agent-runtime-closure-v1" -or $currentProgram -notmatch "current_phase: PHASE12_real-pdf-source-span-vertical-slice" -or $currentProgram -notmatch "latest_completed_program: zuno-lean-complete-product-architecture-v1" -or $currentProgram -notmatch "docs/history/programs/zuno-lean-complete-product-architecture-v1/") {
