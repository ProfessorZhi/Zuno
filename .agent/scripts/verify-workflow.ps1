...
if ($currentProgram -notmatch "zuno-master-architecture-implementation-v1" -or $currentProgram -notmatch "state: active" -or $currentProgram -notmatch "current_phase: PHASE10_eval-observability-langsmith") {
    $failures.Add("current-program.md must declare the active master architecture implementation program and PHASE10 state")
}
