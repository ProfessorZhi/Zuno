...
```text
Product Surface 可以拥有多个 AgentDefinition / AgentVersion / AgentPublication / AgentInstallation。
一次 AgentRun 只能绑定一个 Primary AgentVersion。
一次 AgentRun 只能有一个 Single Controller。
一次 AgentRun 只能有一个 Active PlanVersion。
```

因此“产品支持多个 Agent 资产”不等于“当前 Runtime 支持自治 Multi-Agent”。Agent Core 可以在一个 PlanVersion 内调度多个 Step、多个并行 Branch、多个 Model Role、多个 Capability 和多个 Tool Action，但最终 Plan、Join、Replan、Final Gate、Publication 和 RunOutcome 仍由同一个 Controller 决定。

Future Multi-Agent 的近期可兼容方向是 Single Controller 下的受控 Agent Role 执行：某些 PlanStep 可以引用其他 AgentVersion 作为 Skill-like Role 或策略模板，但这些 Role 不拥有独立 RunOutcome，不扩大权限，不绕过预算、安全、Trace 或 Final Gate。真正自治 Multi-Agent Runtime 必须新建 ADR，重新定义父子 Run、Delegation、权限不放大、预算分摊、冲突仲裁、取消传播和审计链。
