...
docs/modules/                 九个 Target 责任域的 Deep Design V2；02 / 03 进一步进入 Detail Design Candidate V1
...
02 法律领域与工作成果、03 知识与证据已经额外包含 B14.1–B14.8 `Detail Freeze Candidate`。字段、事务、CAS、Migration、Crash Window 或 Failure Injection 任务必须继续读这些小节，但仍要回到 Evidence 判断 Current；不能把 candidate 当作数据库已经实现。
...
当前模块状态是：九篇 `Deep Design V2 / Cross-Module Consistency`；02 / 03 `Detail Design Candidate V1`；`module_detail_freeze: NOT_YET`；`implementation_authorization: NO`。
...
| 02 / 03 冻结前字段、事务、Serving、Crash / Migration 候选 | 对应模块 B14.1–B14.8；仍受总体架构 / ADR 约束 |
