...
| 项目背景、历史事实、立项逻辑 | `docs/project/README.md`、`docs/project/reference.md`、provenance | Project |
...
| 文档故事化 / Human Narrative Rewrite | Project README / `architecture/architecture.md` / 目标 Module README + narrative research | Documentation；先区分 Writing Gap 与 Architecture Gap |
...
| 个人贡献 | `docs/project/README.md` + `reference.md` + provenance + 历史任务级证据 | Project；不得由 Target 或导师成果反推 |
| 文档结构、README、Agent 路由 | `docs/governance/documentation-architecture.md`、Agent workflow、`docs-map.md` | Governance |
| 阅读总体架构、理解系统 | Project README → `architecture/architecture.md` → Modules README | Architecture |
| 总体架构或跨层设计 | `architecture/architecture.md` + `architecture/reference.md` + ADR + Evidence + Governance | Architecture |
| 阅读模块、理解责任域 | `docs/modules/README.md` + 目标模块 `README.md` | Module Design |
| Module Deep Design / Review | `architecture/reference.md` + 目标模块 `reference.md` + 相邻 reference + ADR + Evidence | Module Design |
| 字段、DB / Manifest / Checkpoint / Registry、并发、Migration、Failure Injection | 目标模块 `reference.md` + 当前代码 / Migration；先确认实现授权 | Architecture + Implementation |
| 架构为什么发生变化 | 当前 `architecture/architecture.md` / `reference.md` + 对应 ADR；历史 Red/Blue 仅作辅助考古 | Architecture / Decisions |
...
| `src/backend/zuno` | `code-map.md`、`debugging.md`、Architecture reference + 对应 Module reference | Runtime Owner |
| eval / dataset / metric | `tools/evals/zuno/AGENTS.md`、`verification-map.md`、Evidence、Evaluation reference | Eval Owner |
...
- Architecture Human Narrative / Engineering reference 或 Module README/reference 出现语义冲突时，先修冲突，不能让 Agent 自行挑一份“更方便”的答案。
...
read project / current truth
  ...
  → read overall architecture architecture.md/reference as needed
  → read target module README/reference as needed
  → read adjacent owner references and Evidence
...
```text
Human Narrative readable first
