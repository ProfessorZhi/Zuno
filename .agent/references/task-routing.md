...
| Module Deep Design / Review | 总体架构 Part A+B、目标模块 Part A+B、相关 ADR、Evidence、Governance | Module Design；不自动授权实现 |
| 实现、测试、Migration、Recovery、Contract、Security | 总体架构 + 对应模块 Part A+B + ADR + Evidence + Governance | Architecture / Runtime |
...
- 模块 Deep Design 发现跨模块 Owner 或总体不变量冲突时，停止模块局部设计并升级为 Architecture Gap。
- 发现未提交资产、未进入 main 的 Commit 或未归属文件时，默认保留，不用磁盘占用作为删除理由。
- 需要删除当前树材料时先列精确 manifest，确认用户授权和 Git 可追溯性。
- 未经明确授权，不根据 Red / Blue Proposal 或 module skeleton 实施业务代码。
...
read current truth
  ...
  → design / review module contract when needed
