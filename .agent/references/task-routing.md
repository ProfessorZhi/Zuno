...
| 文档故事化 / Human Narrative Rewrite | Project/Architecture/目标 Module `README.md` + narrative research | Documentation；先区分 Writing Gap 与 Architecture Gap |
...
| 阅读总体架构、理解系统 | Project README → Architecture README → Modules README | Architecture |
| 总体架构或跨层设计 | `architecture/README.md` + `architecture/reference.md` + ADR + Evidence + Governance | Architecture |
...
| 架构为什么发生变化 | 当前 Architecture README/reference + 对应 ADR；历史 Red/Blue 仅作辅助考古 | Architecture / Decisions |
...
- Human README 与 Engineering reference 出现语义冲突时，先修冲突，不能让 Agent 自行挑一份“更方便”的答案。
...
read project / current truth
  ...
  → read overall architecture README/reference as needed
