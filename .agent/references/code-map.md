...
- `src/backend/zuno/knowledge/`：RAG / GraphRAG / Evidence / Citation / retrieval / fusion 薄入口；Program 1B / V2 新增 `knowledge/storage/` 作为 SQLite-compatible durable ingestion store contract；当前包含 `GraphRAGExtractorConfig`、`AgenticRetrievalRouter`、`EvidenceBundle`、`CitationBuilder` 和 `GraphRAGIndexPipelineContract` 等 contract，不表示生产级 GraphRAG runtime 已迁入。
