...
- `src/backend/zuno/knowledge/`：RAG / GraphRAG / Evidence / Citation / retrieval / fusion 薄入口；Program 1B / V2 已把 `knowledge/storage/` 接入 Product V1 local durable ingestion baseline，覆盖 source object、workspace file、parse job / snapshot、document version / blocks、index manifest / chunks 和 citation lineage 的 SQLite / local file store round-trip；当前包含 `GraphRAGExtractorConfig`、`AgenticRetrievalRouter`、`EvidenceBundle`、`CitationBuilder` 和 `GraphRAGIndexPipelineContract` 等 contract，不表示生产级 GraphRAG runtime 已迁入。
