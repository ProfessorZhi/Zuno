...
`.agent/programs/` 当前 active program 是 `zuno-enterprise-document-ingestion-platform-v2`，当前 phase 是 `PHASE02_durable-storage-contract`。最近完成并归档的 program 是：
...
该 program 是 `zuno-enterprise-agentic-graphrag-production-suite-v1` 的 Program 1A。它已完成企业知识库文档解析与索引交接地基：Document IR、parser worker、native parser、adapter boundary、index manifest lineage、workspace ingest -> ParseGateway 闭环、runtime subsystems thread prompts 和 no-active closure。
...
active 状态下，`.agent/programs/` 根目录保留当前 active phase 文件。completed program 的 phase 和 prompt evidence 必须在 `docs/history/programs/` 归档。

## Program Suite 顺序

1. `zuno-production-document-ingestion-and-thread-foundation-v1`
   - 状态：completed / archived。
   - 范围：Document Ingestion、Document IR、parser worker、index handoff、fixtures、runtime subsystems thread prompts。
2. `zuno-enterprise-document-ingestion-platform-v2`
   - 状态：active。
   - 当前 phase：`.agent/programs/PHASE02_durable-storage-contract.md`。
   - 范围：Program 1B / V2；文件对象存储、SQLModel durable store、parse / index job store、queue adapter、local worker runner、OCR / VLM blocked diagnostics、workspace API durable closure、restart recovery tests。
...
4. `zuno-agent-planning-integration-v1`
   ...
   - 范围：合并 Program 3，落地真实 planning / ReAct / reflection / replan。
...
- `docs/history/programs/zuno-production-document-ingestion-and-thread-foundation-v1/`：完成 PHASE01-PHASE08、workspace ingest -> ParseGateway、runtime subsystems prompts、验证和 no-active closure。
...
- 当前 active program 必须按 PHASE01 -> closure 顺序推进。
