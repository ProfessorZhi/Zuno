...
- 当前 Program 1B / V2 PHASE02 已提供 `zuno.knowledge.storage` 最小 durable storage contract，focused test 证明 source object、workspace file、parse job、parse snapshot、document version、index manifest 和 index chunk 可通过 SQLite-compatible store round-trip；它还没有接入 `/workspace/file`、`/workspace/ingest` 主 API，也不是生产 Postgres / object store / queue 平台。
