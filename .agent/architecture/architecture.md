...
| Document Ingestion | PHASE04 已有 Parser Capability Matrix、Canonical Document IR、router contract、adapter registry、deterministic Parse Gateway runtime、job status、legacy chunk normalizer、index handoff 和 parser golden fixture replay；PHASE07 已补本地 parser queue snapshot / metrics / retry 和 external parser adapter target-blocked boundary；PHASE05 已证明 handoff 可进入本地 index job runtime；Program 3 Mega PHASE12 已证明 native text / markdown / csv / json / html / code 可进入本地 parse/index/retrieval E2E，PDF / Office / image/scanned 缺 provider 时 target-blocked 且 no fake index，binary source object 可追溯 sha256 / storage_uri。 | 生产级 parser platform、真实 Docling / MinerU / Unstructured worker、OCR/layout/table/code chunk 深度抽取、生产 queue worker 和 parser operations metrics。 | 第一版 runtime owner surface 与本地 E2E owner surface 已完成，本地 queue / adapter boundary 已定；生产 parser 平台和外部 worker 仍未完成。 | PHASE09/10/12 消费 evidence、citation 和 eval 字段；PHASE13 当前汇总 parser / worker / blocked metrics。 |
...
当前 Zuno 有本地 eval baseline、trace/eval foundation、Contract Review eval 证据、PHASE12 scenario summary / trace summary，以及上一轮 runtime-full program PHASE10 的 `ZunoSpan` / `LangSmithExportAdapter` / `ReleaseEvalBaseline` contract。PHASE13 当前负责把 E2E 输出整理成统一 metrics / regression summary；LangSmith 驱动的持续评测平台、在线采样、持久 trace store 和完整 CI release gate 仍是 Target。这样未来简历或 demo 里可以写出真实数字，而不是只说“做了 RAG 评测”。
