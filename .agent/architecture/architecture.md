...
Zuno 是 **AgentChat 驱动的企业知识库 Agentic GraphRAG Workspace**。用户在聊天里提出目标，系统通过统一输入层、知识库层、模型层、记忆层、工具层、安全层和规划层协作，由 Single Controller Agent 自动决定如何检索、是否使用 GraphRAG、是否调用工具、是否重查或反思，最后生成带引用、可追溯、可评测的回答或 artifact。
...
| 记忆层 / Memory & Context Layer | 管理 working、session、episodic、semantic、procedural、governance memory，并构造 ContextPack。 | 感知为 Agent 记得上下文，但敏感记忆受控。 |
| 工具层 / Tool Control Plane | 管理 ToolCard、ToolRequest、approval、credential broker、sandbox、result normalizer 和 tool trace。 | 感知为工具审批和工具执行结果。 |
| 规划层 / Single Controller Agent Runtime | `prepare_context -> plan -> ReAct -> observe -> reflect -> replan -> post_turn_commit`，决定检索、工具、反思、重查和最终回答。 | 感知为 Agent 正在检索、分析、生成。 |
...
flowchart TB
  ...
  INDEX --> QUERY --> PRODUCT --> CONTEXT --> MEMORY --> AGENT --> MODE --> NEED
  MODE --> NORMAL --> ROUTER
