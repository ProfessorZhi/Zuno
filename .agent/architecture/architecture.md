...
| Document Ingestion | 依赖和零散能力存在；统一 Parse Gateway 还不是 Current。 | 多格式 parser matrix、Canonical Document IR、OCR/layout/table/code chunk、ACL/provenance、index handoff。 | 大差距；企业知识库质量瓶颈在这里。 | PHASE04 先做 parser router、IR、golden fixtures 和 citation anchors。 |
...
Parser matrix 要明确“默认路径、fallback、结构保留、证据锚点、测试 fixture”。目标表：

| 格式 | 默认 parser | fallback | 结构保留 | Evidence anchor |
| --- | --- | --- | --- | --- |
| PDF text | PyMuPDF4LLM / Docling adapter | Unstructured | page、heading、paragraph、table、bbox | page + bbox + block_id |
| Scanned PDF | OCR / VLM adapter | manual review | page image、OCR span、confidence | page + bbox + OCR confidence |
| DOCX | Docling / MarkItDown | Unstructured | heading、paragraph、table、comment | section path + paragraph id |
| PPTX | MarkItDown / Unstructured | VLM summary | slide、shape、speaker note | slide number + shape id |
| XLSX | Unstructured / pandas adapter | CSV extraction | sheet、row、column、table | sheet + cell range |
| TXT / MD | native parser | none | heading、line range、code fence | file path + line range |
| HTML | native / readability parser | Unstructured | DOM section、link、table | DOM path + text span |
| CSV / JSON | native structured parser | text fallback | row id、key path、schema | row/key path |
| Image | OCR / VLM adapter | manual review | OCR span、visual caption、bbox | image id + bbox |
| Code files | native code parser | text fallback | language、symbol、line range | path + line range |
