...
| Document Ingestion | PHASE04 已有 Parser Capability Matrix、Canonical Document IR、router contract、adapter registry、deterministic Parse Gateway runtime、job status、legacy chunk normalizer、index handoff 和 parser golden fixture replay；PHASE07 已补本地 parser queue snapshot / metrics / retry 和 external parser adapter target-blocked boundary；PHASE05 已证明 handoff 可进入本地 index job runtime；Program 3 Mega PHASE12 已证明 native text / markdown / csv / json / html / code 可进入本地 parse/index/retrieval E2E，PDF / Office / image/scanned 缺 provider 时 target-blocked 且 no fake index，binary source object 可追溯 sha256 / storage_uri；PHASE13 已汇总 parser / worker / blocked metrics；PHASE14 已把输入层和文档处理边界写入稳定专题文档。 | 生产级 parser platform、真实 Docling / MinerU / Unstructured worker、OCR/layout/table/code chunk 深度抽取、生产 queue worker 和 parser operations metrics。 | 第一版 runtime owner surface 与本地 E2E owner surface 已完成，本地 queue / adapter boundary 已定；生产 parser 平台和外部 worker 仍未完成。 | PHASE09/10/12/13 消费 evidence、citation 和 eval 字段；PHASE15 当前负责验证归档。 |
