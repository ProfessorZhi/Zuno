...
| Document Ingestion | PHASE04 已有 Parser Capability Matrix、Canonical Document IR、router contract、adapter registry、deterministic Parse Gateway runtime、job status、legacy chunk normalizer、index handoff 和 parser golden fixture replay；PHASE07 已补本地 parser queue snapshot / metrics / retry 和 external parser adapter target-blocked boundary；PHASE05 已证明 handoff 可进入本地 index job runtime；Program 3 Mega PHASE12 已证明 native text / markdown / csv / json / html / code 可进入本地 parse/index/retrieval E2E，PDF / Office / image/scanned 缺 provider 时 target-blocked 且 no fake index，binary source object 可追溯 sha256 / storage_uri；PHASE13 已汇总 parser / worker / blocked metrics。 | 生产级 parser platform、真实 Docling / MinerU / Unstructured worker、OCR/layout/table/code chunk 深度抽取、生产 queue worker 和 parser operations metrics。 | 第一版 runtime owner surface 与本地 E2E owner surface 已完成，本地 queue / adapter boundary 已定；生产 parser 平台和外部 worker 仍未完成。 | PHASE09/10/12/13 消费 evidence、citation 和 eval 字段；PHASE14 当前负责正式文档展开。 |
...
| Eval / Observability | 上一轮 runtime-full program PHASE10 已有 OTel / LangSmith-compatible `ZunoSpan` schema、redacted `ZunoSpanBuilder`、`LangSmithExportAdapter`、`EvalDatasetCase`、`ReleaseEvalBaseline` 和 sandbox audit span bridge；PHASE11 已让 Web trace 面板消费 observability snapshot、trace replay source refs 和 release eval 状态；PHASE12 已生成本地 E2E scenario summary / trace summary，并保留 latency、cost、token、evidence_count、citation_coverage、plan / replan / reflection events；PHASE13 已生成 ConversationRunMetrics、StageMetrics、IngestionMetrics、RetrievalMetrics、PlanningMetrics、SecurityMetrics、CostMetric、EvalComparisonReport 和 regression summary。 | 生产级 LangSmith / OTel sink、在线采样平台、持久化 trace store、完整 RAG/answer/agent/security eval dataset、CI release gates。 | 中等差距；第一版 Web 可见 trace/eval、本地 release baseline、E2E trace summary 和本地 regression summary 已接通，但持续评测台、统一指标面、外部 sink runtime 和 CI operations 未完成。 | PHASE13 已完成 Eval / Trace / Cost / Benchmark baseline；PHASE14 当前负责正式文档展开。 |
