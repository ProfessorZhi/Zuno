...
当前 active program 是 `zuno-production-document-ingestion-and-thread-foundation-v1`，当前 phase 是 `PHASE06_index-handoff-provenance-and-fixtures.md`。这个 program 是 `zuno-enterprise-agentic-graphrag-production-suite-v1` 的 Program 1，目标是关闭企业知识库文档入口 local runtime slice：workspace file -> ParseGateway -> CanonicalDocumentIR -> index handoff -> IndexJobManifest -> retrieval / citation provenance。PHASE01 已完成只读审计，PHASE02 已完成 contract freeze，PHASE03 已完成本地 parser worker lifecycle，PHASE04 已完成 native text / structured file parser baseline，PHASE05 已完成 PDF / Office / OCR / VLM adapter boundary；PHASE06 当前处理 index handoff provenance 和 fixtures。生产 DB、object store、queue / outbox、worker lease、external OCR / VLM 和 external index 仍是 Target，不能写成 Current。
