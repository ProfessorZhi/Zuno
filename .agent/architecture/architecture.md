...
当前 active program 是 `zuno-master-architecture-implementation-v1`，不是上一轮只做图和执行计划的文档 program。它的目标是把目标架构分阶段落地，同时仍然遵守 Current / Target 边界。当前阶段是 `PHASE07_tool-control-plane-mcp-approval`；`PHASE01_program-baseline-and-previous-closure`、`PHASE02_project-folder-and-code-layout-cleanup`、`PHASE03_enterprise-scenario-and-product-loop`、`PHASE04_document-ingestion-parse-gateway`、`PHASE05_agent-runtime-langgraph-harness` 与 `PHASE06_context-memory-system` 已通过 verifier 和 focused tests 证明完成。PHASE07 只负责 ToolCard manifest、executor adapter contract、side-effect risk matrix、approval gate、MCP trust governance 和 result normalizer，不把完整工具审批与 sandbox 平台写成 Current。
