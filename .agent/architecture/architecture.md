...
当前 active program 是 `zuno-production-document-ingestion-and-thread-foundation-v1`，当前 phase 是 `PHASE07_program2-thread-prompts-and-branch-plan.md`。PHASE01 已完成 parser current audit 和 workspace ingest gap 审计；PHASE02 已完成 Document IR、parser adapter contract、document version、source hash、parser config hash、schema version、fallback 和 target-blocked diagnostics 的本地 contract freeze；PHASE03 已完成本地 parser worker / job lifecycle / retry / metrics / snapshot / idempotency；PHASE04 已完成 native text / Markdown / CSV / JSON / HTML / code parser baseline 和 golden fixtures；PHASE05 已完成 PDF / Office / OCR / VLM adapter boundary、dependency probe、privacy / network / budget gate 和 target-blocked evidence；PHASE06 已完成 index manifest parse lineage、diagnostics digest 和 citation lineage handoff。该 program 的任务是先把文档入口和索引交接地基做实，再进入 Program 2 的 Memory / Tool / Security / GraphRAG 并行外围能力。
...
当前 active program 是 `zuno-production-document-ingestion-and-thread-foundation-v1`，当前 phase 是 `PHASE07_program2-thread-prompts-and-branch-plan.md`。这个 program 是 `zuno-enterprise-agentic-graphrag-production-suite-v1` 的 Program 1，目标是关闭企业知识库文档入口 local runtime slice：workspace file -> ParseGateway -> CanonicalDocumentIR -> index handoff -> IndexJobManifest -> retrieval / citation provenance。PHASE01 已完成只读审计，PHASE02 已完成 contract freeze，PHASE03 已完成本地 parser worker lifecycle，PHASE04 已完成 native text / structured file parser baseline，PHASE05 已完成 PDF / Office / OCR / VLM adapter boundary，PHASE06 已完成 index handoff provenance 和 citation lineage；PHASE07 当前生成 Program 2 thread prompts 和 branch plan。生产 DB、object store、queue / outbox、worker lease、external OCR / VLM 和 external index 仍是 Target，不能写成 Current。
