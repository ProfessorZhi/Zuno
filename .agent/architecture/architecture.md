...
当前 active program 是 `zuno-production-document-ingestion-and-thread-foundation-v1`，当前 phase 是 `PHASE08_verification-doc-sync-and-closure.md`。PHASE01 已完成 parser current audit 和 workspace ingest gap 审计；PHASE02 已完成 Document IR、parser adapter contract、document version、source hash、parser config hash、schema version、fallback 和 target-blocked diagnostics 的本地 contract freeze；PHASE03 已完成本地 parser worker / job lifecycle / retry / metrics / snapshot / idempotency；PHASE04 已完成 native text / Markdown / CSV / JSON / HTML / code parser baseline 和 golden fixtures；PHASE05 已完成 PDF / Office / OCR / VLM adapter boundary、dependency probe、privacy / network / budget gate 和 target-blocked evidence；PHASE06 已完成 index manifest parse lineage、diagnostics digest 和 citation lineage handoff；PHASE07 已完成 Program 2 thread prompts、branch / worktree plan、allowed / forbidden paths、focused tests、stop conditions 和主线程合并顺序。PHASE08 负责最终验证、文档同步、自维护审查、归档和下一轮状态交接。
...
| API / 产品闭环 | Completion / knowledge query path foundation 存在；PHASE03 已补 workspace product object、request envelope、stream id contract，并打通 session、file、ingest、task、approval、event/SSE、artifact、feedback 后端 runtime surface；PHASE06 已把 runtime snapshot、approval resume、cancel、共享 lifecycle endpoint、artifact download endpoint / UI、recoverable failure actions 和 feedback_ids 接入 workspace task；PHASE08 已把 tool approval / audit events 接入 workspace task stream；PHASE11 已让 Web workspace Agent 模式消费 file / ingest / task / SSE / approval / artifact / trace-eval / feedback 路径。 | Workspace、task/session、upload、artifact、SSE/WS、download、feedback、trace panel 的生产级 Web/Desktop 闭环。 | 后端 API surface、Web 第一版产品闭环、Web / Desktop 共享 lifecycle 和 artifact 下载已可测；production Desktop 打包/e2e、进程重启后的长任务恢复和外部发布 gate 仍是 Target。 | PHASE03/04/05/06/08/09/10/11 已完成第一版 runtime vertical slice；PHASE12 已完成本地 release closure。 |
...
| Knowledge / Agentic GraphRAG | 已有 `AgenticRetrievalRouter`、`ProductMode` / `QueryMethod`、`StagedFusionPlan`、`EvidenceBundle`、`CitationBuilder`、`UnsupportedClaimChecker`、`GraphRAGIndexPipelineContract` 和 trace payload contract；PHASE05 已有 `KnowledgeIndexRuntime` 本地 BM25 / vector / graph index job、manifest、retry、replay 和 retrieval payload；PHASE07 已补 index adapter contract、manifest provenance / ACL / adapter status；PHASE09 已消费 PHASE05 index runtime 与 evidence、citation、ACL 和 trace 字段；已归档生产成熟化 program PHASE11 已补 source-provenance evidence item、citation source tracing、local RRF/rerank trace、deterministic graph extraction / community report trace、external graph index target-blocked evidence 和 unsupported claim metrics。 | 生产级 multi-channel retrieval、LLM GraphRAG extraction、真实 community report pipeline、生产 reranker、外部 index platform、retrieval / citation eval baseline。 | 中等差距；index runtime 第一版、本地 adapter boundary 和本地 production evidence/citation boundary 已定，但外部图索引、生产 LLM extraction、生产 reranker 与完整 eval runtime 未成熟。 | PHASE11 已关闭本地 Knowledge / GraphRAG / Evidence / Citation runtime boundary；PHASE12 已完成 release closure。 |
...
当前 active program 是 `zuno-production-document-ingestion-and-thread-foundation-v1`，当前 phase 是 `PHASE08_verification-doc-sync-and-closure.md`。这个 program 是 `zuno-enterprise-agentic-graphrag-production-suite-v1` 的 Program 1，目标是关闭企业知识库文档入口 local runtime slice：workspace file -> ParseGateway -> CanonicalDocumentIR -> index handoff -> IndexJobManifest -> retrieval / citation provenance。PHASE01 已完成只读审计，PHASE02 已完成 contract freeze，PHASE03 已完成本地 parser worker lifecycle，PHASE04 已完成 native text / structured file parser baseline，PHASE05 已完成 PDF / Office / OCR / VLM adapter boundary，PHASE06 已完成 index handoff provenance 和 citation lineage，PHASE07 已完成 Program 2 thread prompts 和 branch plan；PHASE08 当前执行最终验证、文档同步、归档和状态交接。生产 DB、object store、queue / outbox、worker lease、external OCR / VLM 和 external index 仍是 Target，不能写成 Current。
