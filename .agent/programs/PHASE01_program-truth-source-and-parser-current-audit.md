...
4. 建立 parser current matrix，至少覆盖 `pdf`、`docx`、`pptx`、`xlsx`、`txt`、`md`、`csv`、`json`、`html`、`image`、`scanned`、`code`。
5. 标记每一项为 `current`、`fallback-current`、`target-blocked` 或 `unknown-needs-test`。
6. 给 PHASE02 输出要冻结的 adapter contract、Document IR 字段和新增测试建议。
