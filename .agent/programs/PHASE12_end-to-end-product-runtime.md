...
- native text / markdown / csv / json / html / code 正常解析并可检索场景。
- PDF 文件在生产 parser 未配置时进入 target-blocked 或 fallback diagnostics，不 fake success。
- DOCX / PPTX / XLSX 文件在 Office parser 未配置时进入 target-blocked 或 fallback diagnostics，不 fake success。
- image / scanned 文件在 OCR / VLM 未配置时 blocked，不创建 fake index。
- binary source object 即使解析 blocked，也能保存 sha256 和 storage_uri/source_ref。
- local queue / worker lifecycle：queued -> parsing -> parsed -> indexing -> indexed。
- dead_letter / reconciler 至少有一个 fixture。
...
- `file_status_timeline`：uploaded、queued、parsing、parsed、indexing、indexed、failed、blocked、dead_letter 或 cancelled 的可见轨迹。
- `parser_dependency_probe`：PDF / Office / OCR / VLM provider 的 present / missing / target-blocked 证据。
- `blocked_reason`：target-blocked 或 policy-blocked 的用户可解释原因。
- `worker_event`：parse worker / index worker 的消费、ack、fail 或 dead_letter 事件摘要。
- `index_status`：indexed / skipped / blocked / failed，并证明 blocked 不创建 fake index。
...
- 主要交付物：完整 E2E scenario tests、shareable scenario summary / trace summary fixture、restart rehydrate proof、standard/deep/deep_without_graph coverage、native format coverage、PDF / Office target-blocked coverage、blocked OCR no fake index、binary object traceability、local queue / worker lifecycle、dead_letter / reconciler fixture、dynamic replan proof、ReflexionLesson candidate proof。
...
- E2E 输出包含 file status timeline、parser dependency probe、blocked reason、worker event、index status 和 citation lineage。
- native text / markdown / csv / json / html / code 至少有一组正常解析并检索 evidence。
- PDF / Office 缺生产 parser 时只能 target-blocked 或 fallback diagnostics，不写 fake success。
- image / scanned 缺 OCR / VLM provider 时 blocked 且没有 fake index。
- binary source object 即使 blocked 也能追溯 sha256 和 storage_uri/source_ref。
- local queue / worker E2E 覆盖 queued -> parsing -> parsed -> indexing -> indexed。
- dead_letter / reconciler 至少有一个 fixture 进入 scenario summary 或 trace summary。
