...
| P11-LC-05 | `src/backend/zuno/knowledge/ingestion/gateway.py` | `canonical_runtime_candidate` | Input / Document Ingestion | ParseGateway local runtime | canonical parser gateway 已有 Package A、PostgreSQL/RabbitMQ/MinIO 和 Human Review gate 部分证据，但当前 Goal02 repair 尚未关闭 | PHASE11 in_progress | CanonicalDocumentIR、ParseSnapshot、typed failure、Package A production runtime | `implementation_available` |
| P11-LC-06 | `src/backend/zuno/knowledge/ingestion/async_runtime.py` | `canonical_runtime_candidate` | Input / Infrastructure | local durable ingestion worker | 作为 local baseline 和 contract evidence；生产默认路径由 Package A PostgreSQL/RabbitMQ/MinIO runtime 承担，当前 Goal02 repair 尚未关闭 | PHASE11 in_progress | LocalQueueBackend、SQLiteDurableIngestionStore、LocalObjectStore 是 local fallback | `local_fallback_contained` |
