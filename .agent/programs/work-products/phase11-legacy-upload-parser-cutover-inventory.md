...
| P11-LC-02 | `src/backend/zuno/platform/services/workspace/attachment_service.py` | `versioned_adapter_required` | Product Surface / Input | workspace attachment prompt builder | 文档附件已显式进入 `versioned.adapter.phase22.chunk_model_projection`，由 ParseGateway 生成 CanonicalDocumentIR 后再转换为过渡文本 excerpt；图片路径继续纳入 Parser Adapter / Review 边界 | PHASE16 removal；PHASE12 迁移旧 Knowledge chunk consumer | `parse_file_into_chunk_model_projection`；`src/backend/zuno/knowledge/ingestion/chunk_projection_adapter.py` | `versioned_adapter_current` |
| P11-LC-03 | `src/backend/zuno/platform/services/pipeline/manager.py` | `versioned_adapter_required` | Input / Knowledge Runtime | knowledge reindex pipeline parse/rag/graph stages | `_parse_chunks` 已改为调用 `versioned.adapter.phase22.chunk_model_projection`：ParseGateway 先生成 CanonicalDocumentIR，再显式转换为旧 RAG/Graph 消费的 ChunkModel；最终 KnowledgeVersion 归 PHASE12 | PHASE16 removal；PHASE12 迁移旧 Knowledge chunk/fact 写入 | `parse_file_into_chunk_model_projection`；`tests/storage/test_pipeline.py` focused path | `versioned_adapter_current` |
