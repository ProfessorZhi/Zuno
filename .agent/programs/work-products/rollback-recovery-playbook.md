...
| Completion legacy rollback | retired after PHASE22 cutover evidence | retired; fail closed | no domain migration; requests must remain on Product Runtime or fail before execution | cannot skip budget/trace disclosure; retired legacy path cannot be used as default fallback | `ZUNO_COMPLETION_CUTOVER_MODE=rollback` is rejected | runtime chain test + retired rollback verifier | P22-T03 |
