...
| Completion legacy rollback | retired after PHASE21 cutover evidence | retired; use explicit cutover drill only | no domain migration; trace missing must be disclosed | cannot skip budget/trace disclosure; retired legacy path cannot be used as default fallback | `ZUNO_COMPLETION_CUTOVER_MODE=rollback` for controlled drill only | runtime chain test + retired env verifier | P22-T03 |
