...
| IndexableDocumentSnapshot Outbox Handoff | 新增 `IndexableDocumentSnapshotV1`、canonical hash、idempotency key、security/delete refs 与 pending outbox；workspace 成功路径先生成 snapshot/outbox，再由本地 consumer index | 仍缺 PostgreSQL production UoW 默认写入、RabbitMQ outbox replay、Knowledge unavailable recovery、review approval 后 handoff，以及真实 duplicate delivery/replay fault evidence | Input / Knowledge Handoff | snapshot handoff outbox | snapshot_ready → outbox_pending → handed_off / replayed | duplicate handoff、knowledge unavailable、outbox replay | `tests/knowledge/test_ingestion_snapshot_handoff.py`; `tests/api/test_workspace_durable_ingest_runtime.py` focused subset | `src/backend/zuno/knowledge/ingestion/handoff.py`; `src/backend/zuno/api/services/workspace_task_runtime.py`; local SQLite snapshot/outbox tables | `target_not_current` |
| Delete / Legal Hold / Restore / Verification | 新增 `DeleteRestoreRuntime`、DeleteLifecycleReceipt 与 SQLite baseline；覆盖 visibility revoke → cleanup request → physical delete → verification、legal hold、duplicate delete、late worker result rejection、restore 不恢复授权；snapshot handoff 可携带 delete refs | 仍缺 PostgreSQL production UoW 默认写入、对象存储真实物理删除、delete during parse/after snapshot 的跨 worker fault evidence、RabbitMQ cleanup/replay 与恢复审批接线 | Input / Security / Knowledge Handoff | delete lifecycle command | visibility_revoked → cleanup_requested → physically_deleted → verified / legal_hold / restored | legal hold、delete after snapshot、duplicate delete、late worker result、restore authorization isolation | `tests/knowledge/test_ingestion_delete_restore.py` | `src/backend/zuno/knowledge/ingestion/delete_restore.py`; local SQLite delete lifecycle table; snapshot delete_refs | `target_not_current` |
