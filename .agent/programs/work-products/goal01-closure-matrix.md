...
| Legacy upload/parser Cutover | default worker uses ParseGateway 的线索存在；活跃入口清单已记录 `/upload`、workspace attachment、knowledge pipeline、legacy doc_parser、ParseGateway 与 local durable runtime；workspace 文档附件默认路径已直接经 ParseGateway 生成 CanonicalDocumentIR 并消费 `CanonicalDocumentIR.blocks`；knowledge pipeline parse stage 也已直接消费 `CanonicalDocumentIR.blocks`；knowledge pipeline graph stage 已直接消费 canonical `graphrag_documents` handoff；当前 `versioned.adapter.phase22.chunk_model_projection` 仅让 knowledge pipeline RAG indexing `_parse_chunks` 先经 ParseGateway 生成 CanonicalDocumentIR，再显式投影为旧 ChunkModel | 仍需迁移 workspace 图片 `_image_to_text`、旧 Knowledge RAG chunk/fact 写入、legacy doc_parser 其他调用，并把默认生产路径接入 SourceObject / PostgreSQL UoW / Quality Gate / IndexableDocumentSnapshot / Outbox；禁止永久 dual-write/dual-runtime/implicit fallback | Input / Product Surface | active upload/parser API and worker entrypoints | legacy_active → canonical / versioned_adapter / removed | hidden fallback、dual runtime、ownerless parser、ChunkModel projection removal | `tests/repo/test_phase11_legacy_upload_parser_cutover.py`; `tests/knowledge/test_legacy_cutover_adapter.py`; `tests/storage/test_pipeline.py` focused subset | cutover inventory：`.agent/programs/work-products/phase11-legacy-upload-parser-cutover-inventory.md`; `tools/scripts/verify_phase11_legacy_upload_parser_cutover.py`; `src/backend/zuno/knowledge/ingestion/chunk_projection_adapter.py`; `docs/evidence/goal05-phase22-workspace-attachment-ir-cutover.md`; `docs/evidence/goal05-phase22-pipeline-graph-ir-cutover.md` | `target_not_current` |
