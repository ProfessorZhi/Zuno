...
- **Cleanup Slice**: P22-T03 cleanup governance guards are active; `src/backend/zuno/knowledge/ingestion/legacy_cutover.py` has retired into `chunk_projection_adapter.py`, workspace attachment default document parsing and Knowledge pipeline parse stage now consume `CanonicalDocumentIR.blocks` directly without `ChunkModel` projection, Knowledge pipeline graph stage now consumes canonical `graphrag_documents` handoff, Knowledge pipeline RAG indexing still calls `parse_file_into_chunk_model_projection` for remaining Milvus/ES consumers, and `/completion` no longer supports rollback into `GeneralAgent.astream()`. Final PHASE22 cleanup remains open until Knowledge pipeline RAG ChunkModel projection retirement, full final verification and program archive complete.
