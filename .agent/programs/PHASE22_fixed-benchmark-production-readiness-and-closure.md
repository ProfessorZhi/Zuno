...
- **Cleanup Slice**: P22-T03 cleanup governance guards are active; `src/backend/zuno/knowledge/ingestion/legacy_cutover.py` and `chunk_projection_adapter.py` have retired, workspace attachment, Knowledge pipeline parse/rag/graph, RAG rebuild script, fixed/local eval and history RAG write default paths now consume Canonical IR / canonical handoff or canonical dict payload without `ChunkModel` projection, Milvus/ES RAG indexing accepts canonical dict payload, and `/completion` no longer supports rollback into `GeneralAgent.astream()`. Final PHASE22 cleanup remains open until old RAG doc_parser / ChunkModel DTO compatibility cleanup, full final verification and program archive complete.
