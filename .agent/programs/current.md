...
state: active
current_phase: `PHASE01_program-boot-baseline.md`
execution_mode: 主线程目标模式 + 默认开启线程内多 agent 协作；不是多线程模式
...
4. `PHASE04_query-router-mode-policy.md`：普通 / 增强 / 自动模式和 `basic / local / global / drift` 方法策略。
5. `PHASE05_context-builder-memory-system.md`：Context Pack、五类记忆、压缩和抽取策略。
6. `PHASE06_capability-toolcard-mcp-system.md`：ToolCard、Native BM25 tool search、MCP/tool policy。
7. `PHASE07_hooks-evidence-trace-artifact-system.md`：Hooks、Evidence Check、Trace、Artifact 证据链。
