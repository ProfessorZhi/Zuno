...
- `PHASE07_fastapi-jwt-auth-compat-retirement-plan.md`：first slice complete。已把 `fastapi_jwt_auth` 降到单文件 compatibility shell，但目录尚未退休。
- `PHASE08_backend-physical-cleanup-slices.md`：active。当前从文档/guardrail 进入真正物理迁移，下一轮两个线程分别处理 resources/compatibility 和 target-layer relocation。
