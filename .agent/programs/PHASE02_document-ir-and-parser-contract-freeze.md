...
2. 让 unknown / unsupported format 返回稳定 diagnostics，而不是抛散乱异常。
3. 为 fallback adapter 增加明确 `fallback=True` 或等价元数据。
4. 为 target-blocked adapter 增加 `blocked_reason`，例如缺外部依赖、缺 OCR engine、缺生产 worker。
5. 更新 ingestion README，只描述已测试 Current 和明确 Target。
6. 运行 focused tests，失败时先修契约层，不扩大到 GraphRAG。
