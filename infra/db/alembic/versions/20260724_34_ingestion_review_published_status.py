"""allow ingestion review published status
...
def upgrade() -> None:
    ...
    op.create_check_constraint(
        ...
        "status in ('created','lease_claimed','running','succeeded','blocked','review_pending','approved','handoff_pending','published','rejected','expired','failed','cancelled','lease_lost','dead_letter','fenced_out')",
    )
    ...
    op.create_check_constraint(
        ...
        "status in ('planned','queued','leased','running','succeeded','blocked','review_pending','approved','handoff_pending','published','rejected','expired','failed','cancelled','dead_letter')",
    )
