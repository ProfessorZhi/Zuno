"""allow ingestion review decision parse statuses

Revision ID: 20260724_32
Revises: 20260724_31
...
def downgrade() -> None:
    ...
    op.create_check_constraint(
        "ck_ingestion_parse_jobs_status",
        "ingestion_parse_jobs",
        "status in ('planned','queued','leased','running','succeeded','blocked','review_pending','failed','cancelled','dead_letter')",
    )
    op.drop_constraint("ck_ingestion_parse_attempts_status", "ingestion_parse_attempts", type_="check")
    op.create_check_constraint(
        "ck_ingestion_parse_attempts_status",
        "ingestion_parse_attempts",
        "status in ('created','lease_claimed','running','succeeded','blocked','review_pending','failed','cancelled','lease_lost','dead_letter','fenced_out')",
    )
