...
class OpenAPIToolAdapter:
    """
    OpenAPI 鈫?LLM Tool Adapter (OpenAPI 3.x)
    """
    ...
    async def execute(self, **kwargs) -> Any:
        ...
        if self.auth_config and self.auth_config.get("auth_type") == "Bearer":
        ...
        *,
        ...
        if missing_required:
            return {
                ...
                "summary": "Missing required test arguments",
                "details": [f"default operation: {operation_id}", f"target: {tested_url}"],
                "warnings": [f"Missing required arguments: {', '.join(missing_required)}"],
                "executed": False,
        ...
        if isinstance(result, dict):
            ...
            result_hint = f"result keys: {preview_keys}" if preview_keys else "result object is empty"
        elif isinstance(result, list):
            result_hint = f"result list length: {len(result)}"
        else:
            result_hint = f"result type: {result_type}"

        return {
            "ok": True,
            "runtime_type": "remote_api",
            "summary": "API connectivity check succeeded",
            "details": [f"tested operation: {operation_id}", f"target: {tested_url}", result_hint],
            "warnings": [],
            "executed": True,
            "operation_id": operation_id,
            "tested_url": tested_url,
            ...
            for method, spec in methods.items():
                ...
                for param in spec.get("parameters", []):
                    if "name" not in param or "in" not in param:
                        raise ValueError(f"Invalid parameter in {operation_id}")
                    if param["in"] not in {"path", "query", "header", "cookie"}:
                        raise ValueError(f"Invalid parameter location in {operation_id}")

                if "requestBody" in spec:
                    content = spec["requestBody"].get("content", {})
                    if not content:
                        raise ValueError(f"requestBody missing content in {operation_id}")
                    if "application/json" not in content:
                        raise ValueError(f"{operation_id} only supports application/json body")
