...
当前面试模式研究入口来自 `ProfessorZhi/internship-work` 的 `interview/00_面试文案仓库/red-team/`。截至已核验的 `f7d0c450...` 基线，其来源审计记录：全量扫描 1,159 份 `正文.md`，内容级分层深读 300 份；数字是该次研究基线，不是永久冻结值。每次用户提示面经有更新时，先读取该仓库最新 `main` 与 `06_来源审计.md`，不得继续引用旧计数。
