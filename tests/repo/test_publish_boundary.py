...
def test_readme_and_roadmap_share_current_program_truth() -> None:
    ...
    assert _current_phase_name(current_program) == "PHASE01_program-truth-source-and-parser-current-audit.md"
    assert "state: active" in current_program
    assert "active_program: zuno-production-document-ingestion-and-thread-foundation-v1" in current_program
