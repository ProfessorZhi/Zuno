...
def test_package_a_duplicate_success_replay_refuses_handoff_quality_mismatch_without_ack(
    monkeypatch,
) -> None:
    import zuno.knowledge.ingestion.production_runtime as production_runtime
    ...
    with pytest.raises(IngestionPersistenceError, match="quality_decision_id"):
        asyncio.run(runtime.process_rabbitmq_delivery(delivery))
...
def test_package_a_duplicate_success_replay_refuses_dead_letter_handoff_without_ack(
    monkeypatch,
) -> None:
    import zuno.knowledge.ingestion.production_runtime as production_runtime
