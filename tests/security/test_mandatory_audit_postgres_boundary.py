...
def _migrated_database(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> tuple[Engine, Engine, str]:
    ...
    # this slice. AUTH-C owns the deployment-entrypoint repair.
...
def _record_secret(engine: Engine, *, tenant_id: str, workspace_id: str, secret_ref: str) -> None:
...
def test_external_effect_requires_durable_mandatory_audit_before_dispatch(
    ...
    try:
        _record_secret(engine, tenant_id=tenant_id, workspace_id=workspace_id, secret_ref=secret_ref)
        ...
        result, receipt = asyncio.run(
            gateway.invoke_readonly(
                ...
                approval=ToolApprovalBinding(
                    ...
                    comment="approved, but no durable audit channel was configured",
                ),
            )
        )
...
def test_external_effect_dispatches_only_after_matching_durable_audit(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine, admin_engine, database_name = _migrated_database(tmp_path, monkeypatch)
    tenant_id = "tenant-audit-ok"
    workspace_id = "workspace-audit-ok"
    trace_id = "trace-mandatory-audit-ok"
    call_id = "effect-mandatory-audit-ok-1"
    secret_ref = "secret-ref:mail:mandatory-audit-ok"
    executor_calls: list[str] = []
    ...
    try:
        ...
        with InfrastructureUnitOfWork(engine, tenant_id=tenant_id) as repo:
            repo.configure_audit_channel(
                ...
                capacity_limit=16,
                owner_id="audit-bootstrap:selected-test",
        ...
        async def executor() -> dict[str, str]:
            executor_calls.append("sent")
            return {"message_id": "remote-message:audit-ok:1"}
        ...
        result, receipt = asyncio.run(
            gateway.invoke_readonly(
                ...
                args={
                    "to": "reviewer@example.com",
                    "body": "audit proof must predate the effect",
                    "secret_ref": secret_ref,
                },
                tenant_id=tenant_id,
                workspace_id=workspace_id,
                ...
                readonly=False,
                approval=ToolApprovalBinding(
                    decision_ref="security-decision:mandatory-audit-ok",
                    adapter_ref="test.approval",
                    comment="approved with configured mandatory audit durability",
                ),
            )
        )
        ...
        with engine.connect() as connection:
            audit = connection.execute(
                text(
                    "SELECT audit_id, channel_id, effect_id, owner_id, status, created_at "
                    "FROM infra_mandatory_audit_events WHERE effect_id = :effect_id"
                ),
                {"effect_id": prepared_id},
            ).mappings().one()
            effect = connection.execute(
                text(
                    "SELECT effect_receipt_id, created_at FROM tool_effect_receipts "
                    "WHERE prepared_tool_action_id = :prepared_id"
                ),
                {"prepared_id": prepared_id},
            ).mappings().one()
        ...
        assert result == {"message_id": "remote-message:audit-ok:1"}
        assert executor_calls == ["sent"]
        assert int(requirement_count) == 1
        assert str(audit["channel_id"]) == AUDIT_CHANNEL_ID
        assert str(audit["effect_id"]) == prepared_id
        assert str(audit["owner_id"]) == f"tool-runtime:{call_id}"
        assert str(audit["status"]) == "durable"
        assert audit["created_at"] <= effect["created_at"]
    finally:
        _drop_database(engine, admin_engine, database_name)
