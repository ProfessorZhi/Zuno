...
def test_platform_rag_uses_canonical_imports() -> None:
    ...
    assert "from zuno.api.dto.search import SearchModel" in contents[paths[1]]
    assert "from zuno.api.dto.rerank import RerankResultModel" in contents[paths[2]]
    assert "from zuno.platform.services.rag.es_client import client as es_client" in contents[paths[3]]
    assert "from zuno.platform.services.storage import storage_client" in contents[paths[4]]
    assert "from zuno.platform.services.rag.vector_db.chroma_client import ChromaClient" in contents[paths[5]]
    assert "from zuno.platform.services.rag.embedding import get_embedding" in contents[paths[6]]
    assert "from zuno.platform.services.rag.vector_db.milvus_lite_client import MilvusLiteClient" in contents[paths[7]]
    assert "from zuno.platform.services.rag.vl_embedding import get_vl_image_embedding, get_vl_text_embedding" in contents[paths[8]]

    retired_paths = [
        "src/backend/zuno/platform/services/rag/parser.py",
        "src/backend/zuno/platform/services/rag/doc_parser",
    ]
