...
def test_fastapi_jwt_auth_runtime_imports_use_vendored_package() -> None:
    ...
    from zuno.platform.compatibility.vendor.fastapi_jwt_auth.auth_config import (
        AuthConfig as VendoredAuthConfig,
    )
    from zuno.platform.compatibility.vendor.fastapi_jwt_auth.auth_jwt import (
        AuthJWT as VendoredAuthJWT,
    )
    from zuno.platform.compatibility.vendor.fastapi_jwt_auth.config import (
        LoadConfig as VendoredLoadConfig,
    )
    from zuno.platform.compatibility.vendor.fastapi_jwt_auth.exceptions import (
