...
# ── V3 final fail-closed closure: duplicated unknown types never raise ────


def test_two_identical_unknown_receipt_types_never_raise():
    # Two identical unknown receipt types must never raise KeyError: the
    # unknown-type handling runs before the known-type duplicate map.
    binding = make_binding()
    binding["receipts"] = list(binding["receipts"]) + [
        _fake_receipt("fake_receipt", "fake-1"),
        _fake_receipt("fake_receipt", "fake-2"),
    ]
    result = VALIDATOR.validate(_rehash(binding))
    assert result.state is BindingValidationState.BLOCKED
    assert result.gap_codes == ("unknown_receipt_type",)
...
def test_duplicated_unknown_receipt_type_reason_deduplicated():
    # Any number of identical unknown receipt types yields the single fixed
    # deduplicated code, never per-receipt or per-type codes.
    binding = make_binding()
    binding["receipts"] = list(binding["receipts"]) + [
        _fake_receipt("mystery_type", f"mystery-{i}") for i in range(4)
    ]
    result = VALIDATOR.validate(_rehash(binding))
    assert result.state is BindingValidationState.BLOCKED
    assert result.gap_codes == ("unknown_receipt_type",)
    for code in result.gap_codes:
        assert code in ALL_GAP_CODES
        assert "mystery_type" not in code


def test_duplicated_unknown_receipt_type_with_secret_value_never_raises():
    secret = "sk-duplicated-secret"
    binding = make_binding()
    binding["receipts"] = list(binding["receipts"]) + [
        _fake_receipt(secret, "fake-1"),
        _fake_receipt(secret, "fake-2"),
    ]
    result = VALIDATOR.validate(_rehash(binding))
    assert result.state is BindingValidationState.BLOCKED
    assert result.gap_codes == ("unknown_receipt_type",)
...
# ── V3 final fail-closed closure: blank runtime_version / snapshot_ref ─────


def test_whitespace_runtime_version_is_blocked():
    result = VALIDATOR.validate(make_binding(runtime_version="   \t \n "))
    assert result.state is BindingValidationState.BLOCKED
    assert result.gap_codes == ("runtime_version_missing",)


def test_whitespace_corpus_snapshot_ref_is_blocked():
    result = VALIDATOR.validate(make_binding(corpus_snapshot_ref="  \t  "))
    assert result.state is BindingValidationState.BLOCKED
    assert result.gap_codes == ("corpus_snapshot_ref_missing",)


def test_blank_runtime_version_is_not_incomparable():
    # A blank top-level runtime_version must never flow into the comparison
    # stages: BLOCKED wins and no receipt mismatch noise is produced.
    binding = make_binding(runtime_version="   ")
    result = VALIDATOR.validate(binding)
    assert result.state is BindingValidationState.BLOCKED
    assert result.gap_codes == ("runtime_version_missing",)


def test_blank_snapshot_ref_is_not_incomparable():
    binding = make_binding(corpus_snapshot_ref=" \t ")
    result = VALIDATOR.validate(binding)
    assert result.state is BindingValidationState.BLOCKED
    assert result.gap_codes == ("corpus_snapshot_ref_missing",)


def test_non_empty_mismatch_remains_incomparable():
    # The frozen INCOMPARABLE rule: only when BOTH sides are non-blank and
    # differ does the comparison stage report INCOMPARABLE.
    binding = make_binding()
    binding["receipts"] = [
        r if r["receipt_type"] != "usage_receipt" else dict(r, runtime_version="rt-9.9")
        for r in binding["receipts"]
