...
def test_is_probably_local_base_url():
    from zuno.evals.rag_eval.run_local_embedding_eval import is_probably_local_base_url
...
def test_infer_provider_from_base_url():
    from zuno.evals.rag_eval.run_local_embedding_eval import infer_provider_from_base_url
...
def test_validate_model_id_accepts_matching_embedding(monkeypatch):
    from zuno.evals.rag_eval.run_local_embedding_eval import validate_model_id

    async def fake_get_llm_by_id(llm_id):
        return {"llm_id": llm_id, "llm_type": "Embedding", "base_url": "http://127.0.0.1:11434/v1"}

    monkeypatch.setattr("zuno.evals.rag_eval.run_local_embedding_eval.LLMService.get_llm_by_id", fake_get_llm_by_id)
...
def test_probe_embedding_model_returns_dimension(monkeypatch):
    from zuno.evals.rag_eval.run_local_embedding_eval import probe_embedding_model

    async def fake_get_embedding(text, config_override=None):
        return [0.1, 0.2, 0.3, 0.4]

    monkeypatch.setattr("zuno.evals.rag_eval.run_local_embedding_eval.get_embedding", fake_get_embedding)
...
def test_resolve_embedding_model_id_auto_picks_local_candidate(monkeypatch):
    from zuno.evals.rag_eval.run_local_embedding_eval import resolve_embedding_model_id
    ...
    monkeypatch.setattr(
        "zuno.evals.rag_eval.run_local_embedding_eval.list_llm_candidates",
...
def test_preflight_auto_pick_local_embedding(monkeypatch):
    ...
    monkeypatch.setattr(
        "zuno.evals.rag_eval.run_local_embedding_eval.resolve_embedding_model_id",
...
def test_preflight_can_use_active_config_embedding(monkeypatch):
    ...
    monkeypatch.setattr(
        "zuno.evals.rag_eval.run_local_embedding_eval.get_active_embedding_config",
...
def test_resolve_direct_local_embedding_registration_registers_new_llm(monkeypatch):
    from zuno.evals.rag_eval.run_local_embedding_eval import resolve_direct_local_embedding_registration
    ...
    monkeypatch.setattr("zuno.evals.rag_eval.run_local_embedding_eval.LLMDao.get_all_llm", fake_get_all_llm)
    monkeypatch.setattr("zuno.evals.rag_eval.run_local_embedding_eval.LLMService.create_llm", fake_create_llm)
...
def test_preflight_accepts_direct_local_embedding_registration(monkeypatch):
    ...
    monkeypatch.setattr(
        "zuno.evals.rag_eval.run_local_embedding_eval.resolve_direct_local_embedding_registration",
        fake_resolve_direct_local_embedding_registration,
    )
    monkeypatch.setattr("zuno.evals.rag_eval.run_local_embedding_eval.LLMService.get_llm_by_id", fake_get_llm_by_id)
    monkeypatch.setattr("zuno.evals.rag_eval.run_local_embedding_eval.list_llm_candidates", fake_list_llm_candidates)
    monkeypatch.setattr("zuno.evals.rag_eval.run_local_embedding_eval.get_embedding", fake_get_embedding)
...
def test_run_local_embedding_eval_falls_back_to_stackless_when_db_is_unavailable(monkeypatch):
    ...
    from zuno.evals.rag_eval.run_local_embedding_eval import run_local_embedding_eval
    ...
    monkeypatch.setattr(
        "zuno.evals.rag_eval.run_local_embedding_eval.preflight_local_embedding_eval",
        fake_preflight_local_embedding_eval,
    )
    monkeypatch.setattr(
        "zuno.evals.rag_eval.run_stackless_local_eval.run_stackless_local_eval",
        fake_run_stackless_local_eval,
    )
    monkeypatch.setattr("zuno.evals.rag_eval.summarize_eval_profiles.summarize", fake_summarize)
    monkeypatch.setattr("zuno.evals.rag_eval.summarize_eval_profiles.write_markdown", fake_write_markdown)
