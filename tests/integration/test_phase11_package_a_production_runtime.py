...
def test_package_a_rejects_parse_job_identity_mismatch_before_inbox() -> None:
    payload = {
        "tenant_id": "tenant-a",
        "workspace_id": "workspace-a",
        "source_object_id": "source:pkg-a:identity",
        "document_version_id": "document-version:pkg-a:identity",
        "parse_plan_id": "parse-plan:pkg-a:identity",
        "parse_job_id": "parse-job:pkg-a:other",
        "object_ref": "s3://bucket/tenant-a/workspace-a/identity.md",
        "object_manifest_ref": "manifest:pkg-a:identity",
        "content_hash": "a" * 64,
        "size_bytes": 12,
        "mime_type": "text/markdown",
        "parser_policy_ref": "parser-policy:pkg-a",
        "quality_policy_ref": "quality-policy:pkg-a",
        "security_decision_ref": "security-decision:pkg-a",
        "security_epoch_ref": "security-epoch:pkg-a:identity",
        "max_attempts": 2,
    }
    envelope = CrossModuleEnvelopeV1(
        contract_name="zuno.ingestion.parse.requested",
        contract_version="v1",
        contract_bundle_version="wave1",
        message_id="outbox:parse-job:pkg-a:identity",
        producer_module="workspace.file_upload",
        consumer_module="ingestion.parser_worker",
        tenant_id="tenant-a",
        workspace_id="workspace-a",
        correlation_id="trace-pkg-a-identity",
        idempotency_key="parse:tenant-a:workspace-a:hash:identity",
        aggregate_type="ParseJob",
        aggregate_id="parse-job:pkg-a:identity",
        effective_security_epoch_ref="security-epoch:pkg-a:identity",
        trace_id="trace-pkg-a-identity",
        data_classification="internal",
        occurred_at="2026-07-20T00:00:00Z",
        created_at="2026-07-20T00:00:00Z",
        payload=payload,
    ...
    with pytest.raises(PackageARejectDeliveryError, match="delivery parse job identity mismatch"):
        asyncio.run(runtime.process_rabbitmq_delivery(delivery))
...
def test_gate_b_redelivery_refuses_dead_letter_handoff_replay_without_ack(monkeypatch) -> None:
    engine = _engine()
    content = b"# Redelivery\nCommitted before ACK, handoff terminal fault."
    envelope = _seed_retryable_job(engine, content=content)
    delivery_payload = CanonicalOutboxDeliveryV1(
        aggregate_id=envelope.aggregate_id or "",
        event_id=envelope.message_id,
    ...
    def _unexpected_parse(_request):
        raise AssertionError("handoff replay fault must not reparse")
        ...
        with engine.begin() as conn:
            conn.execute(
                text(
                    ...
                    SET knowledge_handoff_status = 'dead_letter'
                    WHERE indexable_snapshot_id = :indexable_snapshot_id
                    """
                ),
                {"indexable_snapshot_id": "indexable:parse-job:pkg-a:retry:attempt:1"},
            )

        with pytest.raises(IngestionPersistenceError, match="knowledge_handoff_status"):
            asyncio.run(runtime.process_rabbitmq_delivery(delivery))
...
def test_gate_b_expired_deadline_closes_attempt_and_lease_without_snapshot(monkeypatch) -> None:
    engine = _engine()
    content = b"# Deadline\nDo not parse after deadline."
    envelope = _seed_retryable_job(engine, content=content).model_copy(
        update={"deadline_at": datetime(2026, 7, 19, 12, 0, tzinfo=timezone.utc)}
    )
    delivery_payload = CanonicalOutboxDeliveryV1(
        aggregate_id=envelope.aggregate_id or "",
        event_id=envelope.message_id,
    ...
    def _unexpected_parse(_request):
        raise AssertionError("expired deadline delivery must not call Parser Gateway")
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            assert row["attempt_status"] == "cancelled"
            assert row["failure_code"] == "deadline_expired"
            assert row["lease_state"] == "released"
            assert row["job_status"] == "cancelled"
            assert conn.execute(text("SELECT count(*) FROM ingestion_parse_snapshots")).scalar_one() == 0
            assert conn.execute(text("SELECT count(*) FROM ingestion_indexable_document_snapshots")).scalar_one() == 0
