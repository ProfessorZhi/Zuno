...
> 状态：PHASE01-09 已归档。Program 3 continuation 当前在 `.agent/programs/PHASE10_directory-surface-map-and-guardrails.md`，此目录保存历史证据。
...
Program 3 还不能写成 closure。PHASE10 active，目标是补齐 `src/backend/zuno/DIRECTORY_MAP.md`、目录迁移 skill 和 verifier guardrails，让 `core/`、`services/`、`database/`、`schema/`、`tools/`、`utils/`、`config/` 等剩余 migration source 不再被误读成目标架构主线。
