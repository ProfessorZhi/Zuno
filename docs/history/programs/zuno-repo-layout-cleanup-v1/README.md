...
> 状态：completed / archived。PHASE01-15、Directory Surface Alignment V1 和 final alias surface closure 均已归档；此目录保存 Program 3 历史证据。
...
- 把只剩视觉噪音的 `mcp_servers/`、`middleware/`、`evals/` 顶层兼容壳退休为受控兼容入口，让目录树更贴近目标架构。
...
- Final alias surface closure：把根级 `compatibility.py`、`config.py`、`core.py`、`database.py`、`evals.py`、`mcp_servers.py`、`middleware.py`、`resources.py`、`schema.py`、`services.py`、`settings.py`、`tools.py`、`utils.py` 收口到 `platform/compatibility/legacy_aliases.py`，让包根只剩 `__init__.py`、`main.py` 和六层目录。
...
- `PHASE10_alias-inventory-and-target-contract.md`：重新定义 Program3 最终完成态，明确根级 alias `.py` 文件不再属于完成态。
- `PHASE11_import-smoke-and-compat-registry-design.md`：建立旧 import smoke matrix，设计 legacy alias registry。
- `PHASE12_low-risk-alias-surface-cleanup.md`：收口低风险 alias：`compatibility`、`config`、`evals`、`resources`。
- `PHASE13_medium-risk-alias-surface-cleanup.md`：收口 `mcp_servers`、`middleware`、`database`、`schema`、`tools`、`utils`。
- `PHASE14_high-risk-core-services-settings-cleanup.md`：收口 `core`、`services`、`settings`，不做 runtime architecture upgrade。
- `PHASE15_final-root-surface-guard-and-closure.md`：收紧 verifier / tests，归档 Program3 final alias surface closure。

归档关键词：`backend 六层迁移计划`、`repo hygiene verifier`、`capability/mcp/servers`、`platform/middleware`、`legacy alias registry`。
...
Directory Surface Alignment V1 完成时，顶层文件仍保留 `__init__.py`、`main.py` 和受控 alias module。Final Alias Surface Closure 进一步退休这些根级 alias 文件，旧 public import path 改由 `platform/compatibility/legacy_aliases.py` 注册兼容。

## Final Alias Surface Closure

Program 3 追加完成根级 alias 文件最终收口：
...
旧 public import path 继续可用，但不再通过根级 `.py` alias 文件暴露。兼容入口统一由 `src/backend/zuno/platform/compatibility/legacy_aliases.py` 注册，包括 `zuno.core.*`、`zuno.services.*`、`zuno.database.*`、`zuno.schema.*`、`zuno.tools.*`、`zuno.utils.*`、`zuno.settings`、`zuno.middleware.*`、`zuno.mcp_servers.*`、`zuno.evals.*` 和 `zuno.compatibility.*`。
...
- PHASE10-15 final alias surface closure：main thread goal mode, registry-backed legacy import compatibility, final commit on `main`
