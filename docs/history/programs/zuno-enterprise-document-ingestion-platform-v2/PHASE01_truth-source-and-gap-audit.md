...
确认 Program 1 已归档且不改写历史 evidence；审计当前 `ParseGateway`、`WorkspaceTaskRuntimeService`、`KnowledgeIndexRuntime` 和相关 API / tests 的真实状态，形成 Program 2 的持久化改造输入事实。
...
- 读取并审计 Program 1 归档 closure、当前架构文档和 Program 2 计划。
...
2. Program truth source：把 Program 2 设为 active，当前 phase 设为本文件。
...
- 发现现有代码事实与 Program 2 目标冲突，且无法在 PHASE01 只读审计中安全归类。
...
- 目标文件 `C:\Users\Administrator\.codex\attachments\3ef2df83-fa21-4598-b9e6-6148a4720ec9\goal-objective.md` 仍是 Program 1 的 PHASE01-PHASE08 执行口径；本轮以后续用户目标调整和仓库 active truth source 为准，执行 Program 2 的持久化平台雏形审计。
- 运行模式：主线程挂机，只读审计；runtime 修改从 PHASE02 开始。

### Current Gap Matrix

| 状态面 | 当前代码事实 | 当前状态 | Program 2 gap |
| --- | --- | --- | --- |
