...
| Legacy upload/parser Cutover | default worker uses ParseGateway 的线索存在；活跃入口清单已记录 `/upload`、workspace attachment、knowledge pipeline、ParseGateway、local durable runtime 与已退休 legacy parser；workspace 文档附件默认路径已直接经 ParseGateway 生成 CanonicalDocumentIR 并消费 `CanonicalDocumentIR.blocks`；knowledge pipeline parse stage 也已直接消费 `CanonicalDocumentIR.blocks`；knowledge pipeline graph stage 已直接消费 canonical `graphrag_documents` handoff；knowledge pipeline RAG indexing、RAG rebuild script、fixed/local eval 与 history RAG write 入口已直接消费 canonical `vector_documents` / canonical vector payload 并以 dict payload 写入 Milvus/ES；旧 `rag/parser.py`、`rag/doc_parser/**`、`ChunkModel` DTO 与 legacy normalizer 已退役 | 仍需把默认生产路径接入 SourceObject / PostgreSQL UoW / Quality Gate / IndexableDocumentSnapshot / Outbox；禁止永久 dual-write/dual-runtime/implicit fallback | Input / Product Surface | active upload/parser API and worker entrypoints | legacy_active → canonical / removed | hidden fallback、dual runtime、ownerless parser | `tests/repo/test_phase11_legacy_upload_parser_cutover.py`; `tests/storage/test_pipeline.py` focused subset; `tests/api/test_workspace_attachment_canonical_ir.py` | cutover inventory：`.agent/programs/work-products/phase11-legacy-upload-parser-cutover-inventory.md`; `tools/scripts/verify_phase11_legacy_upload_parser_cutover.py`; `src/backend/zuno/knowledge/ingestion/vector_payload.py`; `docs/evidence/goal05-phase22-workspace-attachment-ir-cutover.md`; `docs/evidence/goal05-phase22-pipeline-graph-ir-cutover.md`; `docs/evidence/goal05-phase22-rag-eval-ir-cutover.md`; `docs/evidence/goal05-phase22-rag-parser-dto-retirement.md` | `target_not_current` |
...
2026-07-20 Package A succeeded replay snapshot/document lineage gate：

```text
IngestionRepository.load_parse_job_replay_receipt now reads parse_snapshot_id and document_version_id for succeeded duplicate/redelivery receipts.
PackageAProductionIngestionRuntime now requires those fields and cross-checks them against Snapshot Handoff replay receipt before ACK.
The focused test proves a handoff receipt bound to a forged document_version_id is not ACKed or rejected.
py_compile passed.
Package A delivery settlement and persistence replay tests passed: 54 passed.
Package A upload replay, retry boundary, and queue worker tests passed: 29 passed.
