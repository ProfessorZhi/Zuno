...
- 参与重构单 Agent Tool Calling 链路，将 MCP Server 的嵌套 Agent 调用改为主 Agent 直连具体 Tool，并支持按用户配置注入 Server 参数。
- 完善 Workspace 工具路由与回归测试，加入 direct route / ReAct fallback，修复自定义 MCP 名称递归和天气参数解析问题。
- 定位 5 条 HotpotQA retrieval smoke 中 GraphRAG 的 Recall@5 回退，优化候选融合与图检索排序，同日复测从 0.80 恢复至 1.00。
- 参与 Context / Memory V2 落地，完善 scope 与调用前上下文读取，将同 scope 的 task summary 和已审核 memory 接入模型上下文。
