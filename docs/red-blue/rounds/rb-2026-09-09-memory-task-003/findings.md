...
本轮复测 `rb-2026-09-09-resume-claims-002` 剩余的任务级 Evidence Gap。v3 Resume 的 Context / Memory 主 bullet 已能由 program boot 记录、PR #8、历史 commit 和 tests 支撑；只保留一个仍会改变面试回答边界的 Finding。

## F1 — 工程任务 Why 已恢复，但真实业务触发与运行结果仍未恢复
...
- eight-deliverables program 启动 commit `0ee4288f28ea664b89ea6ca8164dc811c1a6b9a2` 已经以 `planned` 状态创建 PHASE05，并记录工程 Why：Context / Memory 需要 owner、source ids、compression / extraction 和 review policy，避免增强模式退化成不可验证的 Prompt 拼接；
...
这些证据已经解决上一轮“只能说参与 Memory，无法讲具体任务”的主要 S2 风险，也让 Blue 能解释这条工程任务为什么在 program 中被规划。

### What is still unknown

当前没有恢复：

- PHASE05 对应哪一个真实产品 / 业务问题、客户或用户需求；
- 对应 Issue、法院 / 客户原话或需求单；
- 跨 scope、未审批 Memory、provenance 丢失等 failure mode 是否真实发生过线上事故；
- 该 slice 在真实任务上的质量、效率、成本或用户结果。

因此不能把这条故事包装成“客户发现线上 Memory 污染 → 我定位并修复 → 指标提升”。当前可以准确说成：**program 启动时已经明确 Why 的 planned foundation engineering task + tested failure modes**。
...
1. 工程任务 Why 可以回到 `0ee428...` 的 PHASE05 原始计划；不能继续扩大成某个已恢复的客户 / 法院需求；
...
- task-level `EVIDENCE_GAP`：已经能回答工程 Why、具体文件、状态 / contract、failure mode、tests 和 simpler alternative。
