...
flowchart TB
  ...
  USER["User Workspace"]
  API["API and Session Layer"]
  MODEL["Model Gateway and Policy"]
  ...
  STRUCT["Structured Long Term Memory"]:::mem
  PACK["Model Context Packet"]:::mem
  POST["PostTurn Memory Pipeline"]:::mem
  TOOL["Capability and Tool"]
  KNOW["Knowledge and Retrieval"]
  INGEST["Document Ingestion"]
  PLATFORM["Platform Infrastructure"]
  SECURITY["Security and Policy"]:::guard
  EVAL["Trace and Eval"]:::guard
  ART["Artifact and Workspace"]
  USER --> API --> CORE
  MODEL --> CORE
  CORE --> MEMORY
  MEMORY --> RAW
  ...
  MEMORY --> PLATFORM
  KNOW --> PLATFORM
  class USER,API,MODEL,MEMORY,TOOL,KNOW,INGEST,PLATFORM,ART node;
```
...
- 边界：Security、Trace 和 Eval 是横切治理层，不是业务入口。
...
flowchart LR
  ...
  UI --> HTTP --> SESSION --> CONTEXT --> CORE
  CONTEXT --> MEMORY --> CONTEXT
  CORE --> ROUTER --> DISPATCH
  DISPATCH --> OBS --> CORE
  OBS --> POST --> WRITE --> MEMORY
  POST --> TRACE
  CORE --> TRACE --> STREAM --> UI
  CORE --> EVAL
  CORE --> OUT --> HTTP
  OUT --> UI
...
flowchart LR
  ...
  UPLOAD["Upload Enterprise Docs"]
  PARSE["Parse Gateway"]
  CHUNK["Chunk Metadata ACL"]
  INDEX["Index BM25 Vector Graph"]
  QUERY["User Query"]
  CONTEXT["Context Builder"]
  MEMORY["Selected Memory<br/>summary structured"]
  ...
  MEMSTORE --> CONTEXT
  QUERY --> CONTEXT --> MEMORY --> AGENT --> NEED
  NEED -->|No| ANSWER
  NEED -->|Yes| MODE
  MODE --> BASIC --> EVID
  MODE --> LOCAL --> EVID
  ...
  INDEX --> DRIFT
  EVID -->|retry| CONTEXT
  EVID -->|pass| ANSWER --> TRACE --> MEMCAND --> REVIEW --> MEMSTORE
  class UPLOAD,PARSE,CHUNK,INDEX,QUERY,CONTEXT,MEMORY,BASIC,LOCAL,GLOBAL,DRIFT,EVID,TRACE,MEMCAND,REVIEW,MEMSTORE node;
```
...
flowchart TB
  ...
  CAPABILITY --> CARD["ToolCard Registry"]
  CAPABILITY --> EXEC["Execution Adapter"]
  ...
  ART --> POLICY
  GOV --> POLICY
  class RUNTIME,MEMORY,CAPABILITY,KNOWLEDGE,INGESTION,WORKSPACE,PLAN,RAW,WINDOW,SUMMARY,STRUCT,TYPES,PACK,GOV,CARD,EXEC,RET,DOC,ART node;
```

#### 分析

- 关注点：领域对象和职责。
- Zuno 映射：Runtime、Memory、Tool、Knowledge、Ingestion、Workspace 和 Policy 是目标领域子系统；Memory 是 write-manage-read 子系统，不是聊天历史拼接。
...
flowchart LR
  ...
  API --> SESSION --> CONTEXT
  CONTEXT --> MEMREAD --> MEMSTORE --> CONTEXT
  CONTEXT --> AGENT
  AGENT --> PLANNER --> REACT
  REACT --> TOOLSEL --> TOOLREG
  REACT --> RETROUTER
  INGEST --> RETROUTER
  TOOLREG --> POLICY
  RETROUTER --> POLICY
  ...
  MEMREVIEW --> TRACE
  AGENT --> TRACE
  class API,SESSION,CONTEXT,MEMREAD,MEMSTORE,MEMWRITE,PLANNER,REACT,TOOLSEL,TOOLREG,RETROUTER,INGEST,OBS,CIT,TRACE node;
```

#### 分析

- 关注点：组件和连接器。
- Zuno 映射：控制由 Agent 集中；能力通过 registry、retrieval router、policy guard 和 evidence checker 进入结果；Memory 通过 read policy 进入 Context Pack，通过 post-turn write path 进入 durable memory。
...
flowchart LR
  ...
  GOAL --> PREP --> READMEM --> ROUTE --> PLAN --> STEP --> CALL --> OBS --> RAW --> WORK --> REFLECT
  REFLECT -->|continue| STEP
  REFLECT -->|replan| REPLAN --> PLAN
  REFLECT -->|finish| FINAL --> COMMIT
  COMMIT --> SUMMARY
