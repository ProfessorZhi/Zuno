...
08 持续判断每个新风险边界上，当前主体、资源、用途和政策是否仍然允许继续；登录成功只提供初始身份上下文。已经合法完成的历史读取仍然是历史；权限撤销控制未来动作，不会假装过去从未发生。
...
Target 因而让审批跟稳定 action identity、关键参数摘要、版本和当时安全上下文关联。影响现实或安全语义的内容变化以后，旧 Approval 失去资格，需要重新判断。审批绑定具体风险和具体动作，不形成整个 Run 的永久通行证。
...
如果未来这些需求可以被成熟平台完整承担，而且 Zuno 不再需要额外的法律业务新鲜度、动作绑定或生命周期语义，08 应继续缩薄。逻辑责任只在需要保护权威边界时存在；没有独立语义时应继续合并或复用。
...
**Current：** Selected Evidence 已经证明几个窄但重要的 fail-closed 窗口：SecurityEpoch 在现实 send 前被撤销时 provider 不会 dispatch；SecretRef 在 lease validation 前被撤销时同样不会 dispatch；`MANDATORY_BEFORE_EFFECT` 缺少 committed audit proof 时 executor 为 0，audit commit 后 SecurityEpoch 再次失效也仍会阻止发送。已发送 Effect 的 mandatory-audit row 还会进入 `effect_observed`，避免永久占用 durable capacity；proof 现在绑定 persisted Security AuditRequirement 的 id/hash，requirement 漂移会在 send 前 fail closed。Tool Effect 的 Security fact identity 也不再由 trace 单独决定，同一 action 的 response-loss replay 可以跨 trace 命中同一 durable epoch。Continuous Authorization 的部分门禁已被证明，完整治理闭环仍未建立。

**Gap：** `MANDATORY_BEFORE_EFFECT` 的基本 durable send gate 与 persisted requirement/proof binding 已经闭合；剩余 Audit gap 是 08 是否耐久表达 `BEST_EFFORT / DURABLE / MANDATORY_BEFORE_EFFECT` class、数据库级跨 tenant isolation，以及 proof 已提交但 send 前被阻断或 send 后 crash/restart 时的 lifecycle。Current Tool Effect epoch 已移除 trace-only identity，但 Target composed SecurityEpoch 的 workspace / principal / resource scope persistence仍未实现；production WorkspaceRuntimeComposition / SecurityDecision resolver binding 也尚未建立 Current proof。其余仍需要长任务 resume / retry 后重新授权、模型 egress、Approval invalidation、Secret rotation、Policy Engine outage、Prompt Injection、Memory Recall / No-Recall、Retention / Legal Hold / purge convergence 和生产环境 qualification。
