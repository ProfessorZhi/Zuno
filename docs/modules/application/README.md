...
最容易出现的设计错误，是因为 Application 最靠近用户，就把授权、知识、Runtime、Domain 和外部 Effect 的状态全部复制成本地 `status`。短期看调用方便，长期却会出现两个模块都声称自己知道“真正状态”。01 把其他 Owner 已经成立的事实组合成对外一致的产品行为；授权、知识、Runtime、Domain 和 Effect 仍由各自 Owner 保持最终权威。
...
Agent Version = 产品能力 / 配置版本，它和某一次运行内部的 PlanVersion 属于不同层次。
...
### 一旦交付改变外部世界，Effect truth 必须离开 Application
...
发布需要在边界上重新消费当前事实。普通答案发布前重新确认当前安全和来源资格；正式 WorkProduct 则读取 02 的当前版本有效性。已经交付的历史不被改写，但新的下载、展示或继续使用可以被当前规则阻断。
