...
### 七类正式对象不是七张表，而是一条可以追溯的业务链

把 Matter、DocumentVersion、Claim、Evidence、Finding、HumanDecision 和 WorkProduct 分开，并不是为了把数据库画得更复杂，而是为了让一个正式成果可以沿业务因果一路往回解释。

一份 WorkProduct 可以引用一个或多个 Finding；Finding 依赖正式 Evidence 和 Claim；Evidence 回到不可变 DocumentVersion；HumanDecision 记录专业人员对候选结果做过怎样的接受、修改或拒绝；所有这些对象最终都受同一个 Matter 范围约束。这样，当用户问“这份结论当时为什么成立”时，系统不需要去猜某次模型 Prompt，而是能够沿正式依赖链找到当时业务上真正采用的依据。

这种依赖不要求所有关系都变成一张巨大的图数据库。第一阶段更重要的是稳定 identity、version、dependency 和 provenance；底层用关系表、关联表还是其他实现，要等详细设计和性能证据再定。

### 两个人同时修改同一事项时，为什么不能最后写入的人覆盖前一个人

法律工作里很容易出现并发：一名业务人员正在修改 Finding，另一名人员刚刚接纳了新 Evidence，后台 Agent 也可能同时完成一个候选分析。如果三方都基于同一个旧版本直接写回，最后一次写入会静默吞掉前面的业务变化。

因此正式准入需要带 expected prior DomainVersion（预期前置领域版本）或等价的并发条件。提交时如果发现领域已经前进，就返回明确的版本冲突。调用方重新读取当前事实以后，可以决定重新合并候选、重新规划，或者要求人工处理。

这种冲突不是普通数据库错误，也不应该靠自动 Retry 把旧 payload 再写一遍。因为冲突已经说明“你基于的业务世界变了”，真正需要的是重新判断，而不是再试一次 SQL。

### 一份新证据为什么只应该影响真正依赖它的结果

如果一个事项有几百个 Finding，新材料到来以后默认“全案全部重跑”最简单，但成本高，而且会把原本稳定的结果也重新暴露给模型波动。相反，如果系统完全不记录正式依赖，就又无法知道哪些结果应该复核。

目标架构因此要求正式 Evidence、Finding 和 WorkProduct 之间保留足够的依赖信息。新 Evidence 被接纳以后，先沿依赖找到直接受影响的 Finding，再向上找到相关 WorkProduct；能够证明影响范围时做有界重新评估，证明不了时宁可扩大复核范围或交人工，也不能假装自己知道影响只发生在某一处。

这也是为什么“检索排名变化”不能直接让正式结果失效。只有新的正式材料、正式证据或其他已经进入领域的依赖变化，才能改变正式结果的当前有效性；知识模块和评测模块可以发出风险信号，但不能越过领域边界修改历史事实。

### 删除、法律保全和历史可追溯性怎样同时存在

法律业务有时要求删除数据，有时又存在 Legal Hold（法律保全）或审计保留义务。领域模块不能简单把所有对象加一个 `deleted=true`，也不能因为某个 Store 暂时还保留字节，就继续允许业务召回和使用。

08 安全与治理负责形成当前有效的生命周期政策：是否允许继续召回、是否必须保留、何时可以物理清除。02 负责在自己的持久化边界执行这些政策，并保存执行事实。已经正式存在过的 WorkProduct、HumanDecision 或 Admission causation 如果依法需要保留历史，就不能为了让“删除状态看起来干净”而破坏审计链；如果政策要求未来禁止召回，也不能因为历史记录仍在而继续把它当成当前可用数据。
