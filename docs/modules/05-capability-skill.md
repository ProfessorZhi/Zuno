...
一个论文算法或实验脚本不能因为“效果不错”就直接进入产品。它需要形成明确能力：例如事件抽取、事件对齐、冲突检测、事实—法条对应、类案检索、法律适用性或证据推理。

每个能力至少要知道自己的版本、输入输出、来源和适用范围，以及当前 Provider 是否满足这个 Contract。Provider 可以更换，但调用方不应该因为更换模型或服务就重新理解整个业务语义。

### 输出为什么只能是候选

专业能力可以说“这里可能存在冲突”“这条材料可能支持某个主张”“这些法条可能适用”，但它不能直接宣布一个 Finding 已经成为正式法律事实。它输出的是候选、观察、引用或建议，后续还要经过证据、权限、质量和必要人审。

这条边界让能力模块可以大胆使用模型和研究算法，同时不让不确定性直接污染 Canonical Domain State。
...
如果所有专业能力都散落在 Agent Prompt、Tool wrapper 和业务服务里，Planner 无法知道 Executor 真正能做什么，测试也无法区分是能力退化还是运行控制错误。独立能力边界让 Planner 可以基于真实能力范围生成可执行 Step，也让研究成果有明确的工程化出口。
...
### B4 Provider Boundary

Provider 可以是本地算法、LLM、fine-tuned model、OSS、API、MCP 或其他服务。Capability Contract 归本模块；网络、进程和存储只是实现。Provider drift 必须显式暴露，不能静默改变语义。
