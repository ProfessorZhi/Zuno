...
- `schema_registry.py` 为 31 张领域表和 14 张基础设施表登记唯一模块 Owner，并覆盖全部 revision；ScriptDirectory 证明只有一个 head、无 branch label、revision registry 无缺漏。
- 空库从 base 升级到 `20260718_12`，重复 upgrade 无副作用；downgrade base 删除全部 45 张托管表，再次 upgrade 完整重建。
