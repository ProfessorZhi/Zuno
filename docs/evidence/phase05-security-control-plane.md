...
- `PostgresSecurityProductActionGuard` 和 `SecurityProductActionRequest` 提供 Product/API action 重新授权端口；workspace artifact read/download、workspace approval resume 与 MCP admin 管理 override 已接入 guard，生产 `init_config()` 默认配置 `PostgresSecurityProductActionGuard(engine)`，artifact read/download/resume 调用路径会重新授权，deny 时返回 403 且 resume 不会越过 `approval_waiting`；MCP admin override deny 时会在 DAO 写入/删除前中断。
- 最小 eval evidence 覆盖：
...
- Security fault matrix 仍未完整覆盖 Product/API citation 默认路径和非 MCP admin 面。
