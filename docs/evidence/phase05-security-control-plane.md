...
- `PostgresSecurityProductActionGuard` 和 `SecurityProductActionRequest` 提供 Product/API action 重新授权端口；workspace artifact read/download 已接入 guard，生产 `init_config()` 默认配置 `PostgresSecurityProductActionGuard(engine)`，artifact read/download 调用路径会重新授权，deny 时返回 403。
- 最小 eval evidence 覆盖：
...
- Security fault matrix 仍未完整覆盖 Product/API resume、citation 和 admin 默认路径。
