...
- Knowledge cleanup confirmation 现在对同一 `cleanup_ref` 重放保持幂等，不重复 history；不同 `cleanup_ref` 即使在后续终态也会 fail closed，不会被终态 replay 吞掉。
...
- `execute_cleanup()` 必须检查 `ObjectVerificationPort` 返回的 `absent=true`，并持久化该端口返回的 `verification_ref`；`absent=false` 会 fail closed，不能把 MinIO delete 成功冒充为领域 verified。
- MinIO object manifest 只在真实对象删除成功或对象已缺失可重建为 deleted 时更新，避免对象删除失败却把 manifest 标为 deleted。
- restore 必须经过 fresh authorization；即使是重复 restore 也必须重新提交同一授权引用才返回幂等结果，缺失授权、撤销授权或不同授权引用会 fail closed。
...
- Restore authorization focused regression：`tests/integration/test_phase11_ingestion_persistence_runtime.py::test_ingestion_delete_restore_coordinator_deletes_verifies_and_restores_real_object` 为 `1 passed in 9.94s`，覆盖 restored 后缺失 fresh authorization 被拒绝、相同授权重复 restore 幂等。
- Delete / restore unit contract：`7 passed in 8.87s`。
- PHASE04 MinIO object store regression：`3 passed in 2.17s`。
- Alembic head：`20260724_34 (head)`。
- Alembic upgrade：最新 schema head 为 `20260724_34 (head)`。
