...
本切片不声明 PHASE22 completed，不声明 fixed benchmark measured，不声明 production ready，也不声明全仓 `ChunkModel` DTO 已退休。旧 RAG doc_parser、历史服务和部分兼容测试仍可使用 `ChunkModel`，但 Knowledge pipeline 默认 parse/rag/graph 入口已退出 `versioned.adapter.phase22.chunk_model_projection`。
...
- `src/backend/zuno/knowledge/ingestion/chunk_projection_adapter.py` 仍存在，但不再是 workspace attachment 或 Knowledge pipeline 默认入口。
