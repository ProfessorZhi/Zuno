...
当前 restore 证据只覆盖真实 `pg_dump`、隔离临时数据库 `pg_restore`、MinIO restore point、infrastructure primitive rows 和恢复库 runtime restart。它证明 restore 在隔离目标中验证后仍不会自动切生产。

本证据不证明完整 Backup/Restore/PITR、RecoverySet 或 official Checkpointer restore，也不关闭 PHASE04。
