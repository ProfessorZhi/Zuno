...
- `tests/legacy_guards/test_hardening01_community_runtime_contract.py` 迁出 legacy guard suite，进入 canonical `tests/repo/test_hardening01_community_runtime_contract.py` 位置，改为 canonical `zuno.platform.services` import，并纳入 PHASE22 cleanup verifier 扫描。
- `tests/legacy_guards/test_phase02_contract_review_asset_migration.py` 迁出 legacy guard suite，进入 canonical `tests/repo/test_phase02_contract_review_asset_migration.py` 位置，改为 canonical `zuno.platform.services` import，并纳入 PHASE22 cleanup verifier 扫描。
- `tests/legacy_guards/test_phase6_agent_graphrag_pluginization.py` 迁出 legacy guard suite，进入 canonical `tests/repo/test_phase6_agent_graphrag_pluginization.py` 位置，改为 canonical `zuno.agent.core` import，并纳入 PHASE22 cleanup verifier 扫描。
- `legacy_general_agent_completion_rollback` 退役：`CompletionService.resolve_cutover_mode()` 不再接受 `ZUNO_AGENT_RUNTIME=legacy_general_agent` 隐式回退，feature flag registry 默认状态改为 `RETIRED`，API compatibility matrix 不再把旧 env rollback 写成 current contract，PHASE22 cleanup verifier 新增机器检查；显式 `ZUNO_COMPLETION_CUTOVER_MODE=rollback` 只保留给受控 cutover drill。
...
python -m compileall -q tests/repo/test_phase3_community_graphrag_v1.py tests/repo/test_zuno_runtime_chain_guard.py passed
17 passed in 14.97s
PHASE22 cleanup boundary verification passed.

python -m compileall -q tests/repo/test_phase4_knowledge_config_v2_and_local_eval.py tests/repo/test_zuno_runtime_chain_guard.py passed
13 passed in 17.63s
PHASE22 cleanup boundary verification passed.
...
python -m compileall -q tests/repo/test_hardening01_community_runtime_contract.py tests/repo/test_zuno_runtime_chain_guard.py passed
16 passed in 4.81s
PHASE22 cleanup boundary verification passed.

python -m compileall -q tests/repo/test_phase02_contract_review_asset_migration.py tests/repo/test_zuno_runtime_chain_guard.py passed
15 passed in 0.46s
PHASE22 cleanup boundary verification passed.

python -m compileall -q tests/repo/test_phase6_agent_graphrag_pluginization.py tests/repo/test_zuno_runtime_chain_guard.py passed
13 passed in 19.93s
PHASE22 cleanup boundary verification passed.

python -m compileall -q src/backend/zuno/api/services/completion.py tools/scripts/phase02_compatibility_runtime.py tools/scripts/verify_phase22_cleanup_boundary.py tests/api/test_completion_unified_runtime.py tests/repo/test_phase02_compatibility_runtime.py tests/repo/test_phase22_cleanup_boundary.py passed
PHASE02 executable compatibility controls passed.
PHASE22 cleanup boundary verification passed.
...
2026-07-30 legacy guard suite retirement:
```text
tests/legacy_guards/ absent in working tree
PHASE02 temporary allowlist no longer keeps tests/legacy_guards/** alive
PHASE01 legacy bypass inventory no longer keeps tests/legacy_guards/** alive
PHASE22 cleanup verifier checks that tests/legacy_guards/ cannot reappear
python -m compileall -q tools/scripts/phase02_compatibility_runtime.py tools/scripts/verify_phase22_cleanup_boundary.py tests/repo/test_phase02_compatibility_runtime.py tests/repo/test_phase22_cleanup_boundary.py passed
python tools/scripts/phase02_compatibility_runtime.py --check passed
python tools/scripts/verify_phase22_cleanup_boundary.py passed
pytest -q tests/repo/test_phase02_compatibility_runtime.py tests/repo/test_phase22_cleanup_boundary.py -p no:cacheprovider passed
5 passed in 0.27s
...
`src/backend/zuno/platform/vendor/README.md`, `src/backend/zuno/platform/common/README.md`, `src/backend/zuno/platform/security/README.md`, `src/backend/zuno/capability/tools/README.md`, `src/backend/zuno/knowledge/README.md`, `src/backend/zuno/knowledge/ingestion/README.md`, `src/backend/zuno/agent/README.md`, `src/backend/zuno/agent/core/README.md`, and `src/backend/zuno/platform/compatibility/README.md` now use migration-compatibility wording for old import surfaces.
