...
本文只证明本次修复切片：Knowledge search API 在受控 QueryService adapter 成功返回后，不再因为当前缺少 ACTIVE Knowledge Snapshot 而把 API 响应降为 500；query run 和 retrieval round 会持久化 retriever expected/used、partial retrieval、no-result 和 retrieval semantics；同时 `zuno.capability.runtime.__all__` 收回到 PHASE14 target surface，不再把 legacy approval adapter 常量和内部 security sink protocol 暴露为公共模块边界。
...
- Durable query-run persistence 现在把 `retrievers_expected`、`retrievers_used`、`retriever_availability`、`partial_retrieval`、`no_result` 和 `retrieval_semantics` 写入 `start_query_run.request_payload` 与 `start_retrieval_round.retriever_set`；`basic` 查询缺少 vector 且没有 evidence/citation 时会记录 `partial_no_result`，不再把 partial retriever 和 no-result 混成普通完成。
