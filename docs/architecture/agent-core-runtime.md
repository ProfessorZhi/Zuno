...
- `SQLiteAgentRunStore` 已提供 PHASE04 SQLite-backed run/checkpoint/event/interrupt/plan/observation/tool claim baseline，并已用新 store 实例证明 pending interrupt 可恢复后 resume；PHASE11 又在 Workspace task 层证明 unified runtime snapshot 可由新的 `UnifiedAgentRuntimeService` 实例从 SQLite store 恢复。
- `UnifiedAgentRuntimeService` 已提供 PHASE05 unified LangGraph runtime skeleton：`build_agent_graph`、`RuntimeNode` routes、store-backed checkpoint/event、stream、approval / ask_user interrupt、resume、cancel 和 `get_snapshot`。该 skeleton 使用 deterministic node adapters，已移除 `status: simulated` marker；后续 PHASE06-PHASE11 已逐步接入 Plan/ReAct step、Tool Control Plane、corrective retrieval、grounded synthesis、Memory / Reflexion 和产品 API/SSE cutover baseline。
...
- PHASE07 已把 `ToolStepExecutor` 接入 Tool Control Plane，approval/resume/idempotency/blocked observation baseline 已完成。
