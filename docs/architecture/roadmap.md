...
| Capability ToolCard 与 MCP | active program / PHASE06 active | 工具层必须能解释候选来源、权限、成本、健康状态和 fallback，而不是只暴露函数列表。 | ToolCard registry、Native BM25 capability search、MCP/local tool policy 和 selection trace 进入代码 contract 与 focused tests。 |
