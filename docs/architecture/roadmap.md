...
| Runtime Upgrade Integration | active program / PHASE09 active | 把前面各 foundation slice 收束进 GeneralAgent 主路径和六层 owner 边界，而不是停留在漂亮 facade。 | 先做只读 runtime call-chain 审计，再实现最小可验证集成点；不默认改 API/SSE/DB/frontend。 |
