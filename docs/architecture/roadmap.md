...
当前没有 active program。Program 3 `zuno-repo-layout-cleanup-v1` 已完成并归档，它完成了 root/docs hygiene、backend 六层迁移计划、六层 README 边界说明、repo hygiene verifier 和 repo tests 收口。
...
| 文件夹继续分门别类 | completed / archived | 让第一次看仓库的人能从目录名直接读出架构，而不是在 `core`、`services`、`rag`、`retrieval`、`graphrag` 之间拼图。 | 已完成 root/docs hygiene、backend 六层 facade-first 迁移计划、六层 README 边界说明、repo hygiene verifier 和 repo tests。 |
...
当前没有 active program。后续 program 草案在 `.agent/architecture/future/programs/`，打开时必须迁入 `.agent/programs/` 并从 `PHASE01` 重新编号。queued 只表示候选顺序，不表示 active。
...
3. `zuno-repo-layout-cleanup-v1`：已完成，归档到 `docs/history/programs/zuno-repo-layout-cleanup-v1/`。完成整个仓库目录、根目录必要入口、docs、`.agent`、tools/tests、生成物隔离和 `src/backend` 六层目标表达的第一轮收口。
...
Program 3 没有实施 runtime、API、DB schema、frontend 或 eval baseline 修改。
...
- `zuno-repo-layout-cleanup-v1` PHASE01-05 已完成并归档。
