...
| Launchable Prototype Target | 一两天内可由 Codex 真实补到闭环、可给个人 / 小团队 / 实验室 / 内网试用的企业级雏形。 | Program 3 Mega 当前 active，目标是 launchable enterprise Agentic GraphRAG product baseline：文档展开、closure archive 和 full verification；已完成的 PHASE12 E2E scenario summary / trace summary 与 PHASE13 regression summary 是 PHASE14 / PHASE15 的输入。 |
...
| Observability / Eval | `ZunoSpan`、release baseline、local eval runner、trace replay surface、no-active release archive、PHASE12 scenario summary / trace summary、PHASE13 ConversationRunMetrics、StageMetrics、IngestionMetrics、RetrievalMetrics、PlanningMetrics、SecurityMetrics、CostMetric、EvalComparisonReport、regression summary 和 full verification evidence。 | PHASE14 / PHASE15 当前目标是把 PHASE13 本地 regression summary 写入正式架构展开、closure archive 和 final verification。 | 外部 LangSmith / OTel sink、Prometheus dashboard、online eval、持久 trace store、CI release gate operations。 |
