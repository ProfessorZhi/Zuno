...
第一版 in-process runtime slice
  ...
  + 文档输入层 SQLite-compatible durable store contract round-trip
...
| 层级 | 定义 | 当前状态 |
| --- | --- | --- |
| Current Local Slice | 代码、focused tests、trace / eval 或 verifier 已证明的本地 runtime 事实。 | Program 1A 已完成 `workspace file -> ParseGateway -> CanonicalDocumentIR -> index handoff -> IndexJobManifest -> retrieval / citation provenance`；Program 1B / V2 PHASE02 已新增文档输入层 SQLite-compatible storage contract round-trip。 |
...
- `README.md` 和 `.agent/programs/current.md`：记录当前 `.agent/programs/` 处于 active 执行态，当前 program 是 `zuno-enterprise-document-ingestion-platform-v2`，最近完成 program 是 `zuno-production-document-ingestion-and-thread-foundation-v1`，归档位置是 `docs/history/programs/zuno-production-document-ingestion-and-thread-foundation-v1/`。
...
| --- | --- | --- | --- |
| Document Ingestion | Program 1A 已完成 Parse Gateway runtime、adapter registry、Document IR、parser golden fixtures、本地 parser queue snapshot / retry / metrics、workspace ingest -> ParseGateway -> index manifest lineage、external parser adapter target-blocked boundary；Program 1B / V2 PHASE02 已完成 source object、workspace file、parse snapshot、document version、index manifest 和 index chunk 的 SQLite-compatible store contract round-trip。 | Program 1B / V2 后续：workspace file durable registry 接入 API、parse job / attempt persistence 接入 ParseGateway、document block / IR artifact persistence、index chunk rehydrate、local queue runner、retry / cancel / replay API、restart recovery。 | 生产级 Docling / MinerU / Unstructured worker、OCR / layout / table / code 深度抽取、生产 queue worker、Postgres、object store、queue / outbox、worker lease、dead letter、reconciler、parser operations metrics。 |
| Index / GraphRAG | 本地 BM25 / vector / graph index job runtime、manifest、retry、replay、retrieval payload、index adapter contract、manifest provenance / ACL / adapter status、local evidence provenance、citation source tracing、local RRF / rerank trace、deterministic graph extraction / community report trace、external graph index target-blocked evidence。 | Index manifest、index chunks、citation_lineage 可从 durable store rehydrate；Agent 问答可使用持久化 index chunk 生成 cited artifact。 | 外部 Elasticsearch / Milvus / Neo4j、生产 LLM GraphRAG extraction、真实 community report pipeline、生产 reranker 服务、index service operations。 |
| Agent Runtime | Controller-node durable runtime surface、checkpoint、interrupt、resume、cancel、failure snapshot、local JSON persistence payload、restart resume contract、exactly-once tool boundary ids。 | Agent task 能消费 durable retrieval evidence，artifact、events、feedback 和 trace summary 可恢复。 | 生产级 LangGraph-compatible DB persistence、跨进程 / 跨 worker 恢复、分布式 exactly-once tool execution。 |
