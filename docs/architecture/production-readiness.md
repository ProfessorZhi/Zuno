...
- `README.md` 和 `.agent/programs/current.md`：记录当前 no-active 状态，最近完成 program 是 `zuno-production-architecture-and-deliverables-completion-v1`。
...
| --- | --- | --- |
| Document Ingestion | Parse Gateway runtime、adapter registry、Document IR、parser golden fixtures、本地 parser queue snapshot / retry / metrics、external parser adapter target-blocked boundary。 | 生产级 Docling / MinerU / Unstructured worker、OCR/layout/table/code 深度抽取、生产 queue worker、parser operations metrics。 |
