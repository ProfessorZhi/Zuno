...
| Agent Runtime | Controller-node durable runtime surface、checkpoint、interrupt、resume、cancel、failure snapshot、local JSON persistence payload、restart resume contract、exactly-once tool boundary ids；Program 2 已证明 workspace task、events、artifact content/ref 和 feedback 可 rehydrate。 | Agent task 消费 durable retrieval evidence 的长任务恢复、trace summary 持久化和跨进程 runner boundary。 | 生产级 LangGraph-compatible DB persistence、跨进程 / 跨 worker 恢复、分布式 exactly-once tool execution。 |
| Memory | SQLModel-backed memory store、governance ledger、promotion、decay、consolidation、GeneralAgent 接入、local deterministic semantic fallback、scoped privacy delete、sensitive context exclusion、memory eval baseline。 | Program 2 不扩 Memory；后续 Runtime Subsystems 再接 DB-backed、可审计、可删除、可评测 memory。 | 生产级 semantic / vector Memory DB、后台 scheduler、企业隐私删除平台、nightly memory eval 和深度 PII / secret 检测。 |
