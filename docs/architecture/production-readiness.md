...
第一版 in-process runtime slice
  ...
  + local E2E product baseline：upload/register、local queue/worker、parse/index、standard/deep/deep_without_graph retrieval、cited artifact、feedback、restart rehydrate、scenario summary 和 trace summary
...
| 层级 | 定义 | 当前状态 |
| --- | --- | --- |
| Current Local Slice | 代码、focused tests、trace / eval 或 verifier 已证明的本地 runtime 事实。 | Program 1 已完成 `workspace file -> ParseGateway -> CanonicalDocumentIR -> index handoff -> IndexJobManifest -> retrieval / citation provenance`；Program 2 / `zuno-enterprise-document-ingestion-platform-v2` 已完成 local durable ingestion baseline：`/workspace/file` source object、`/workspace/ingest` parse / document / index persistence、task / event / artifact / feedback persistence 和 restart recovery；Program 3 Mega 已完成 PHASE03-PHASE13 的本地 ingestion async、retrieval profile、Memory & Context、Capability / Skill / Tool / MCP、Security / Governance、Model Gateway / Cost / Latency、Planning Contract / Strategy Selector、AgentControlRuntime / Reflection / Dynamic Replan / Reflexion、Workspace Product API / Frontend Minimal Sync、End-to-End Product Runtime baseline 和 Eval / Trace / Cost / Benchmark regression summary baseline。 |
| Launchable Prototype Target | 一两天内可由 Codex 真实补到闭环、可给个人 / 小团队 / 实验室 / 内网试用的企业级雏形。 | Program 3 Mega 已完成并归档为 launchable enterprise Agentic GraphRAG product baseline；PHASE12 E2E scenario summary / trace summary、PHASE13 regression summary、PHASE14 稳定专题文档和 PHASE15 full verification / no-active archive closure 是当前证据。 |
...
- `README.md` 和 `.agent/programs/current.md`：记录当前 `.agent/programs/` 处于 no-active 状态，最近完成 program 是 `zuno-launchable-enterprise-agentic-graphrag-full-closure-v1`，归档位置是 `docs/history/programs/zuno-launchable-enterprise-agentic-graphrag-full-closure-v1/`。
...
| Agent Runtime | Controller-node durable runtime surface、checkpoint、interrupt、resume、cancel、failure snapshot、local JSON persistence payload、restart resume contract、exactly-once tool boundary ids；Program 2 已证明 workspace task、events、artifact content/ref 和 feedback 可 rehydrate；Program 3 Mega PHASE09 已提供 local deterministic Strategy Selector、PlannerOutput、PlanState、RetrievalPlan、CapabilityPlan、ReflectionVerdict、ReplanDecision、Reflexion candidate 和 planning trace events；Program 3 Mega PHASE10 已提供 local AgentControlRuntime，可消费 planner output 与 governed observations，执行 step trace、reflection gate、dynamic replan trajectory change、tool_failed / security_blocked stop path、ReflexionLesson pending review path 和 answer_finalized trace；Program 3 Mega PHASE11 已把 product retrieval profile、plan/reflection/replan summary、artifact citation refs、file status 和 frontend API types 接入 Workspace Product API；Program 3 Mega PHASE12 已用真实 local runtime path 证明 E2E scenario summary、trace summary、feedback 和 restart rehydrate；Program 3 Mega PHASE13 已把 PHASE12 E2E 输出汇总为 Eval / Trace / Cost / Benchmark baseline；Program 3 Mega PHASE14 已把 Agent Core runtime 边界写入稳定专题文档；Program 3 Mega PHASE15 已完成 full verification 和 no-active archive closure。 | 产品面只暴露标准检索 / 深度检索，不暴露内部 basic/local/global/drift。 | 生产级 LangGraph-compatible DB persistence、跨进程 / 跨 worker 恢复、分布式 exactly-once tool execution。 |
...
| Observability / Eval | `ZunoSpan`、release baseline、local eval runner、trace replay surface、no-active release archive、PHASE12 scenario summary / trace summary、PHASE13 ConversationRunMetrics、StageMetrics、IngestionMetrics、RetrievalMetrics、PlanningMetrics、SecurityMetrics、CostMetric、EvalComparisonReport、regression summary、PHASE14 eval observability and cost 文档和 PHASE15 full verification evidence。 | 本地 release / trace / eval / cost baseline 已可作为 launchable prototype gate。 | 外部 LangSmith / OTel sink、Prometheus dashboard、online eval、持久 trace store、CI release gate operations。 |
...
| 7 | Security / Trace / Eval / Release | input / retrieval / tool / output gates、redaction、ZunoSpan、release baseline、trace replay surface、local release archive、ConversationRunMetrics、StageMetrics、RetrievalMetrics、PlanningMetrics、SecurityMetrics、CostMetric、EvalComparisonReport 和 full verification evidence。 | Program 3 Mega 的近期目标是把 PHASE13 本地 regression summary 写入正式架构展开、closure archive 和 final verification；长期目标是外部 LangSmith / OTel sink、online eval、持久 trace store、CI release gate operations。 |
