...
| --- | --- | --- | --- |
| Document Ingestion | Program 1A 已完成 Parse Gateway runtime、adapter registry、Document IR、parser golden fixtures、本地 parser queue snapshot / retry / metrics、workspace ingest -> ParseGateway -> index manifest lineage、external parser adapter target-blocked boundary。 | Program 1B / V2：source object store、workspace file durable registry、SQLModel / SQLite store、parse job / attempt persistence、document version / block / IR artifact persistence、index job / chunk persistence、local queue runner、retry / cancel / replay API、restart recovery。 | 生产级 Docling / MinerU / Unstructured worker、OCR / layout / table / code 深度抽取、生产 queue worker、Postgres、object store、queue / outbox、worker lease、dead letter、reconciler、parser operations metrics。 |
