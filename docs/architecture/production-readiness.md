...
| 层级 | 定义 | 当前状态 |
| --- | --- | --- |
| Current Local Slice | 代码、focused tests、trace / eval 或 verifier 已证明的本地 runtime 事实。 | Program 1 已完成 `workspace file -> ParseGateway -> CanonicalDocumentIR -> index handoff -> IndexJobManifest -> retrieval / citation provenance`；Program 2 已完成 local durable ingestion baseline：`/workspace/file` source object、`/workspace/ingest` parse / document / index persistence、task / event / artifact / feedback persistence 和 restart recovery；Program 3 Mega 已完成 PHASE03-PHASE12 的本地 ingestion async、retrieval profile、Memory & Context、Capability / Skill / Tool / MCP、Security / Governance、Model Gateway / Cost / Latency、Planning Contract / Strategy Selector、AgentControlRuntime / Reflection / Dynamic Replan / Reflexion、Workspace Product API / Frontend Minimal Sync 和 End-to-End Product Runtime baseline。 |
| Launchable Prototype Target | 一两天内可由 Codex 真实补到闭环、可给个人 / 小团队 / 实验室 / 内网试用的企业级雏形。 | Program 3 Mega 当前 active，目标是 launchable enterprise Agentic GraphRAG product baseline：Eval / Trace / Benchmark、regression summary、文档展开、closure archive 和 full verification；已完成的 PHASE12 E2E scenario summary / trace summary 是 PHASE13 的输入。 |
...
| Observability / Eval | `ZunoSpan`、release baseline、local eval runner、trace replay surface、no-active release archive、PHASE12 scenario summary / trace summary 和 full verification evidence。 | PHASE13 当前目标是生成 ConversationRunMetrics、StageMetrics、IngestionMetrics、RetrievalMetrics、PlanningMetrics、SecurityMetrics、CostMetric、EvalComparisonReport 和 regression summary；PHASE12 已让 parse / index / ingest status、attempt diagnostics、dead letter、reconciler、restart recovery、blocked no fake index 和 binary traceability 可查询。 | 外部 LangSmith / OTel sink、Prometheus dashboard、online eval、持久 trace store、CI release gate operations。 |
