...
| Agent Runtime | Controller-node durable runtime surface、checkpoint、interrupt、resume、cancel、failure snapshot、local JSON persistence payload、restart resume contract、exactly-once tool boundary ids；Program 2 已证明 workspace task、events、artifact content/ref 和 feedback 可 rehydrate。 | Agentic Retrieval Planner 消费每个知识库的标准检索 / 深度检索 profile、index capability、ACL 和证据质量，自动决定 query rewrite、retriever selection、GraphRAG expansion、reflection、re-query 和 cited answer；Agent task 消费 durable retrieval evidence 的长任务恢复、trace summary 持久化和跨进程 runner boundary。 | 生产级 LangGraph-compatible DB persistence、跨进程 / 跨 worker 恢复、分布式 exactly-once tool execution。 |
