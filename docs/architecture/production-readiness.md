...
| 4 | 架构功能完整实现 | 第一版 runtime-first vertical slice 已完成：Web 产品闭环、Web / Desktop 共享 task lifecycle、artifact 下载、recoverable failure surface、本地 parse / index / retrieval / tool / trace / eval surface、本地 parser queue snapshot、index adapter boundary、local durable restart/resume 和 exactly-once id boundary、focused tests 和 release evidence。 | 生产级目标架构完整落地：生产 parser worker queue、深度解析、外部索引、LangGraph / DB persistence、semantic/vector memory、真实 sandbox、vault、外部 trace/eval、完整 Desktop 包装和 CI release gate。 |
