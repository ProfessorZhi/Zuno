...
当前 active program 是 Program 3 Mega：`zuno-launchable-enterprise-agentic-graphrag-full-closure-v1`，当前 phase 是 `PHASE11_workspace-product-api-frontend-sync.md`。最近完成并归档的 program 是 `zuno-enterprise-document-ingestion-platform-v2`，归档在 `docs/history/programs/zuno-enterprise-document-ingestion-platform-v2/`。当前已具备本地 Model Gateway / Cost / Latency、Planning Contract / Strategy Selector 和 ReAct / Reflection / Dynamic Replan / Reflexion runtime baseline；成熟度和 runtime-first 交付物口径以 `production-readiness.md` 为准。只写 contract、schema 或 README 不能关闭 runtime phase 的规则继续保留。
