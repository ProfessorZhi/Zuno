...
`docs/architecture/` 是 Zuno 当前面向人的正式架构入口。这里保持少而精：一个充实的总架构 Markdown、一个生产成熟度基线、少量稳定专题文档、一个由 Markdown 生成的 HTML 展示页、正式附件和仍然生效的 ADR。
...
当前 `.agent/programs/` 是 no-active 状态。最近完成并归档的 program 是 Program 3 Mega：`zuno-launchable-enterprise-agentic-graphrag-full-closure-v1`，归档在 `docs/history/programs/zuno-launchable-enterprise-agentic-graphrag-full-closure-v1/`。当前已具备本地 Model Gateway / Cost / Latency、Planning Contract / Strategy Selector、ReAct / Reflection / Dynamic Replan / Reflexion runtime、Workspace Product API / Frontend Minimal Sync baseline、PHASE12 End-to-End Product Runtime baseline、PHASE13 Eval / Trace / Cost / Benchmark regression summary baseline、PHASE14 Docs / Architecture Expansion baseline 和 PHASE15 full verification / no-active archive closure；成熟度和 runtime-first 交付物口径以 `production-readiness.md` 为准。只写 contract、schema 或 README 不能关闭 runtime phase 的规则继续保留。
...
3. `document-ingestion-foundation.md`：企业知识库文档入口契约，说明 workspace file、ParseGateway、Document IR、index handoff、幂等、版本、防丢、ACL、citation lineage 和多模态解析边界。
4. `agent-core-runtime.md`、`capability-and-skill-layer.md`、`agentic-retrieval-planner.md`、`eval-observability-and-cost.md`、`input-layer-and-document-processing.md`、`knowledge-space-product-configuration.md`：PHASE14 稳定专题说明，只承载 Current / Target 边界和长期可复用架构语义，不承载 phase log。
5. `architecture.html`：图形化展示页，适合快速看完整架构图，不替代 Markdown 正文。
6. `repo-ownership-matrix.md`：PHASE02 的目录 ownership、compat/vendor 边界和 provider 分类事实表；它是代码布局治理的正式 Current 证据。
7. `.agent/architecture/architecture.md` 与 `.agent/architecture/architecture.html`：Agent 侧镜像，必须由 `tools/agent/render_architecture.py --write` 同步。
8. `decisions/README.md`：仍影响当前主线的正式架构决策。
...
- PHASE14 专题文档只写稳定架构语义、Current / Target 边界和证据路径，不写高频 phase 执行日志。
