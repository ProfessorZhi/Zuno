...
当前 `.agent/programs/` 处于 no-active 状态。最近完成并归档的 program 是 Program 3 Mega：`zuno-launchable-enterprise-agentic-graphrag-full-closure-v1`，归档位置是 `docs/history/programs/zuno-launchable-enterprise-agentic-graphrag-full-closure-v1/`。Program 3 Mega 已完成 PHASE01-PHASE15：truth source / merge plan、shared contract freeze、enterprise ingestion async infrastructure、knowledge retrieval profile、Memory & Context Engine、Capability / Skill / Tool / MCP、Security / Governance、Model Gateway / Cost / Latency、Planning Contract / Strategy Selector、AgentControlRuntime / Reflection / Dynamic Replan / Reflexion、Workspace Product API / Frontend Minimal Sync、End-to-End Product Runtime、Eval / Trace / Cost / Benchmark、Docs / Architecture Expansion 和 full verification / no-active archive closure。Program 2 `zuno-enterprise-document-ingestion-platform-v2` 已归档在 `docs/history/programs/zuno-enterprise-document-ingestion-platform-v2/`；Program 1 `zuno-production-document-ingestion-and-thread-foundation-v1` 已归档在 `docs/history/programs/zuno-production-document-ingestion-and-thread-foundation-v1/`。原 Program 3-6 已合并进 Mega Program；旧 queued 文件只保留在归档中作为 merged inputs。
...
| Trace / Eval | runtime trace、LangSmith 映射、dataset、offline / online eval、retrieval / answer / tool / security 指标。 | 当前有上一轮 runtime-full program PHASE10 `ZunoSpan`、LangSmith export adapter、dataset / baseline contract 和 redacted failure evidence；已归档生产成熟化 program PHASE12 完成本地 release archive、full verification 和 no-active closure；Program 3 Mega PHASE12 已生成 scenario summary / trace summary，PHASE13 已汇总 ConversationRunMetrics、StageMetrics、IngestionMetrics、RetrievalMetrics、PlanningMetrics、SecurityMetrics、CostMetric、EvalComparisonReport 和 regression summary；LangSmith 产品化、在线采样、持久 trace store 和 CI release gate operations 仍是 Target。 |
...
| Document Ingestion | PHASE04 已有 Parser Capability Matrix、Canonical Document IR、router contract、adapter registry、deterministic Parse Gateway runtime、job status、legacy chunk normalizer、index handoff 和 parser golden fixture replay；PHASE07 已补本地 parser queue snapshot / metrics / retry 和 external parser adapter target-blocked boundary；PHASE05 已证明 handoff 可进入本地 index job runtime；Program 3 Mega PHASE12 已证明 native text / markdown / csv / json / html / code 可进入本地 parse/index/retrieval E2E，PDF / Office / image/scanned 缺 provider 时 target-blocked 且 no fake index，binary source object 可追溯 sha256 / storage_uri；PHASE13 已汇总 parser / worker / blocked metrics；PHASE14 已把输入层和文档处理边界写入稳定专题文档；PHASE15 已完成 full verification。 | 生产级 parser platform、真实 Docling / MinerU / Unstructured worker、OCR/layout/table/code chunk 深度抽取、生产 queue worker 和 parser operations metrics。 | 第一版 runtime owner surface 与本地 E2E owner surface 已完成，本地 queue / adapter boundary 已定；生产 parser 平台和外部 worker 仍未完成。 | PHASE09/10/12/13 消费 evidence、citation 和 eval 字段；Program 3 Mega 已归档。 |
| Security / Governance | 上一轮 runtime-full program PHASE10 已有 `InputSecurityGate`、`RetrievalSecurityGate`、`ToolSecurityGate`、`OutputSecurityGate`、`ToolSecurityProfile`、`SandboxAuditEvent`、redaction helper 和 workspace task security event；PHASE11 已在 Web trace 面板展示 security / failure 状态；已归档生产成熟化 program PHASE10 已补本地网络策略、credential ref 和 approval ledger redaction 边界；Program 3 Mega PHASE07 已补 `SecurityTraceSummary`、planner-facing `ask_user` / `replan` / `refuse` action、citation-low / unsupported-claim output verdict 和 filtered retrieval metrics；PHASE12 已在 E2E 中证明 blocked parser / OCR / VLM 只进入 target-blocked evidence，不创建 fake index。 | 真实 sandbox runtime、生产级 approval UI、credential broker、network proxy、生产级 DLP、security eval baseline。 | 中等差距；runtime gate、trace summary 和本地 tool governance 已补强，但隔离执行、外部治理平台和生产 DLP 未完成。 | PHASE07 已关闭本地 Governance Envelope baseline；PHASE09/10/12/13 继续消费 verdict、action 和 metrics。 |
| Eval / Observability | 上一轮 runtime-full program PHASE10 已有 OTel / LangSmith-compatible `ZunoSpan` schema、redacted `ZunoSpanBuilder`、`LangSmithExportAdapter`、`EvalDatasetCase`、`ReleaseEvalBaseline` 和 sandbox audit span bridge；PHASE11 已让 Web trace 面板消费 observability snapshot、trace replay source refs 和 release eval 状态；PHASE12 已生成本地 E2E scenario summary / trace summary，并保留 latency、cost、token、evidence_count、citation_coverage、plan / replan / reflection events；PHASE13 已生成 ConversationRunMetrics、StageMetrics、IngestionMetrics、RetrievalMetrics、PlanningMetrics、SecurityMetrics、CostMetric、EvalComparisonReport 和 regression summary；PHASE14 已把 eval observability and cost 边界写入稳定专题文档；PHASE15 已完成 full verification。 | 生产级 LangSmith / OTel sink、在线采样平台、持久化 trace store、完整 RAG/answer/agent/security eval dataset、CI release gates。 | 中等差距；第一版 Web 可见 trace/eval、本地 release baseline、E2E trace summary、本地 regression summary 和稳定专题文档已接通，但持续评测台、统一指标面、外部 sink runtime 和 CI operations 未完成。 | Program 3 Mega 已归档。 |
...
当前 Zuno 有本地 eval baseline、trace/eval foundation、Contract Review eval 证据、PHASE12 scenario summary / trace summary、PHASE13 metrics / regression summary，以及上一轮 runtime-full program PHASE10 的 `ZunoSpan` / `LangSmithExportAdapter` / `ReleaseEvalBaseline` contract。PHASE13 已把 E2E 输出整理成统一 metrics / regression summary；LangSmith 驱动的持续评测平台、在线采样、持久 trace store 和完整 CI release gate 仍是 Target。这样未来简历或 demo 里可以写出真实数字，而不是只说“做了 RAG 评测”。
...
当前 `.agent/programs/` 处于 no-active 状态，最近完成并归档的 Program 3 Mega 是 `zuno-launchable-enterprise-agentic-graphrag-full-closure-v1`，归档位置是 `docs/history/programs/zuno-launchable-enterprise-agentic-graphrag-full-closure-v1/`。Program 2 `zuno-enterprise-document-ingestion-platform-v2` 已归档，位置是 `docs/history/programs/zuno-enterprise-document-ingestion-platform-v2/`。它承接 Program 1 的企业知识库文档入口 local runtime slice，并把 source object、workspace file、parse job / snapshot、document version / blocks、index manifest / chunks、citation lineage、workspace task、events、artifact content/ref、feedback 和 restart recovery 推进到 SQLite / local file store backed Product V1 durable baseline。Program 1 `zuno-production-document-ingestion-and-thread-foundation-v1` 的 PHASE01-PHASE08 证据保留在历史归档；Program 2 不改写 Program 1 事实。Program 3 Mega 已完成本地 Model Gateway / Cost / Latency、Planning Contract / Strategy Selector、AgentControlRuntime / Reflection / Dynamic Replan / Reflexion、Workspace Product API / frontend types、E2E Product Runtime、Eval / Trace / Cost / Benchmark、文档和架构展开、full verification 和 no-active archive closure。生产 DB、object store、queue / outbox、worker lease、external OCR / VLM、external model provider 和 external index 仍是 Production Scale Target，不能写成 Current。
...
PHASE14 新增的六篇专题文档不是旧 `current-architecture.md` / `target-architecture.md` / `roadmap.md` 的回流。它们只承载稳定专题边界：Agent Core Runtime、Capability and Skill Layer、Agentic Retrieval Planner、Eval / Observability / Cost、Input Layer and Document Processing、Knowledge Space Product Configuration。高频 phase 执行细节仍放在 `.agent/programs/`。
...
- 改 PHASE14 专题文档时，同步 `docs/architecture/README.md`、`.agent/references/docs-map.md`、`.agent/references/architecture-docs-map.md`、verifier 和 repo tests；专题文档必须保留 Current / Launchable / Production Scale 边界。
...
- 当前已有 RuntimeTurnLedger、当前轮 trace reset、GeneralAgent 最小 evidence chain、post-turn evidence payload、六层目标入口 import guard、eval diagnostics、Program 3 Mega PHASE10 的 AgentControlRuntime / Reflection / Dynamic Replan / Reflexion candidate 本地闭环，以及 PHASE12 的 Workspace Product E2E 接入；完整产品级 LangGraph runtime upgrade、分布式恢复和外部 release gate 仍是 Target。
