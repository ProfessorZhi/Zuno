...
当前 active program 是 `zuno-production-document-ingestion-and-thread-foundation-v1`，当前 phase 是 `PHASE04_native-text-and-structured-file-parsers.md`。PHASE01 已完成 parser current audit 和 workspace ingest gap 审计；PHASE02 已完成 Document IR、parser adapter contract、document version、source hash、parser config hash、schema version、fallback 和 target-blocked diagnostics 的本地 contract freeze；PHASE03 已完成本地 parser worker / job lifecycle / retry / metrics / snapshot / idempotency。该 program 的任务是先把文档入口和索引交接地基做实，再进入 Program 2 的 Memory / Tool / Security / GraphRAG 并行外围能力。
...
当前 active program 是 `zuno-production-document-ingestion-and-thread-foundation-v1`，当前 phase 是 `PHASE04_native-text-and-structured-file-parsers.md`。这个 program 是 `zuno-enterprise-agentic-graphrag-production-suite-v1` 的 Program 1，目标是关闭企业知识库文档入口 local runtime slice：workspace file -> ParseGateway -> CanonicalDocumentIR -> index handoff -> IndexJobManifest -> retrieval / citation provenance。PHASE01 已完成只读审计，PHASE02 已完成 contract freeze，PHASE03 已完成本地 parser worker lifecycle；PHASE04 当前强化 native text / structured file parsers。生产 DB、object store、queue / outbox、worker lease、external OCR / VLM 和 external index 仍是 Target，不能写成 Current。
