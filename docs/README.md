...
`docs/` 是 Zuno 的正式项目知识入口。为了让项目故事、架构、模块设计和当前证据不再分散，当前树保留少数稳定入口：
...
7. [模块架构](./modules/README.md)：九个责任域怎样展开、事实归谁、推荐先设计什么；随后按需要进入各模块正文。
...
- `modules/` 只展开已经冻结的九个逻辑责任域；当前模块正文是 design skeleton，不自动意味着 module detail frozen 或 implementation authorized。
