...
- PHASE22 in progress：Goal05 已冻结 removal candidates，并完成 Product Runtime 默认路径中 `zuno.schema.workspace` 与 Product command service `zuno.database import engine` 两个 legacy alias import 的 canonical cutover；证据为 `docs/evidence/goal05-phase22-cleanup-start.md`。它不代表 legacy-free tree、fixed benchmark completed 或 production ready。
