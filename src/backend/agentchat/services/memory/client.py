...
class AsyncMemory(MemoryBase):
    ...
    async def _add_to_vector_store(
        ...
        if not infer:
            ...
            for message_dict in messages:
                ...
                msg_embeddings = await asyncio.to_thread(self._get_embedding_model().embed, msg_content)
        ...
        async def process_fact_for_search(new_mem_content):
            embeddings = await asyncio.to_thread(self._get_embedding_model().embed, new_mem_content)
            new_message_embeddings[new_mem_content] = embeddings
            existing_mems = await asyncio.to_thread(
                self._get_vector_store().search,
    ...
    async def get(self, memory_id):
        ...
        """
        memory = await asyncio.to_thread(self._get_vector_store().get, vector_id=memory_id)
        if not memory:
            return None
    ...
    async def _get_all_from_vector_store(self, filters, limit):
        memories_result = await asyncio.to_thread(self._get_vector_store().list, filters=filters, limit=limit)
        actual_memories = (
            memories_result[0]
    ...
    async def _search_vector_store(self, query, filters, limit, threshold: Optional[float] = None):
        embeddings = await asyncio.to_thread(self._get_embedding_model().embed, query)
        memories = await asyncio.to_thread(
            self._get_vector_store().search, query=query, vectors=embeddings, limit=limit, filters=filters
        )
    ...
    async def update(self, memory_id, data):
        ...
        embeddings = await asyncio.to_thread(self._get_embedding_model().embed, data)
    ...
    async def delete_all(self, user_id=None, agent_id=None, run_id=None):
        ...
        memories = await asyncio.to_thread(self._get_vector_store().list, filters=filters)
    ...
    async def _create_procedural_memory(self, messages, metadata=None, llm=None, prompt=None):
        ...
        try:
            if llm is not None:
                ...
                procedural_memory = await asyncio.to_thread(self._get_llm().invoke, input=parsed_messages, config=None)
        except Exception as e:
        ...
        embeddings = await asyncio.to_thread(self._get_embedding_model().embed, procedural_memory)
    ...
    async def _update_memory(self, memory_id, data, existing_embeddings, metadata=None):
        ...
        try:
            existing_memory = await asyncio.to_thread(self._get_vector_store().get, vector_id=memory_id)
        except Exception:
    ...
    async def _delete_memory(self, memory_id):
        ...
        await asyncio.to_thread(self._get_vector_store().delete, vector_id=memory_id)
        await asyncio.to_thread(
            self.db.add_history,
            memory_id,
            prev_value,
...
class LazyMemoryClient:
    ...
    def _get_client(self) -> AsyncMemory:
...
memory_client = LazyMemoryClient()
