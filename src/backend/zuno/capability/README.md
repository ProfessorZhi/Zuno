...
PHASE02 的 provider 分类入口是 `docs/governance/repo-ownership-matrix.md` 和 `CAPABILITY_TOOL_PROVIDER_CLASSIFICATIONS` / `CAPABILITY_MCP_SERVER_CLASSIFICATIONS`。新增 tool 或 MCP server 目录前，必须先声明分类、target owner、compat path、测试和 verifier。
