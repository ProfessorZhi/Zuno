...
当前底层对象位于 `src/backend/zuno/platform/services/memory/layers.py`；`zuno.services.memory.layers` 只通过 compatibility shell 保留旧读取面，不是当前 owner。PHASE09 的 Current 是可导出、可重建、可审查、可按 scope 删除的本地 memory runtime surface，并带 deterministic semantic fallback；它不是 production-grade Memory DB，也不是外部向量记忆库、长期后台 consolidation job、跨备份隐私删除平台或 nightly memory eval 平台。
...
- 禁止破坏 `zuno.services.memory.*` 旧 import path 作为兼容面，但不要把它误写成当前 owner。
