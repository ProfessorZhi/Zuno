...
`src/backend/zuno/system_skills/` 当前保存 runtime skill resource 和系统技能相关内容。它是应用资源，不是 Agent 工作流里的 `.agent/references/` skill，也不是仓库维护模板。

## Target role

目标状态下，系统技能作为 Capability 或 Agent resource 被选择、加载和注入；目录本身可以继续作为 app-resource，直到 capability skill loading、resource path 和 focused tests 证明可以迁移。
