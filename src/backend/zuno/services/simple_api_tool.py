...
class AgenticRemoteApiDraft(BaseModel):
    ...
    auth_type: Literal[
        "none", "bearer", "basic", "api_key_query", "api_key_header"
    ] = "none"
    api_key_name: str = ""
...
def build_openapi_schema_from_simple_config(
    ...
    operation: dict[str, Any] = {
        ...
        "description": (
            simple_api_config.description
            or simple_api_config.summary
            or simple_api_config.operation_id
        ),
    ...
    if (
        simple_api_config.method in {"POST", "PUT", "PATCH"}
        and simple_api_config.body_schema
    ):
...
async def build_remote_api_assist_draft_agentic(
    req: RemoteApiAssistReq,
) -> RemoteApiAssistResp:
    docs_urls = _resolve_docs_urls(req.docs_url, req.docs_urls)
    if not docs_urls:
        raise ValueError(
            "请至少提供一个文档链接。Agent 模式需要先读取文档正文，再判断接口地址、认证方式和参数。"
        )
    ...
    if len(docs_text.strip()) < 200:
        ...
        raise ValueError(
            f"文档正文不足，Agent 无法判断接口配置。请补充更多文档链接、OpenAPI/Swagger 链接，或提供一条 curl 示例。{detail}"
        )
    ...
    try:
        ...
        raise ValueError(
            "Agent 没能从当前文档判断完整表单。请补充更具体的接口文档、OpenAPI/Swagger 链接，或 curl 示例。"
            f"原因：{err}"
        ) from err

    return _build_remote_api_assist_resp_from_agent_draft(
        req, docs_analysis, agent_draft
    )
...
def build_remote_api_assist_draft(req: RemoteApiAssistReq) -> RemoteApiAssistResp:
    ...
    return _build_remote_api_assist_draft_from_analysis(
        req, docs_urls, docs_analysis
    )
...
def _build_remote_api_assist_draft_from_analysis(
    ...
    raw_url = (
        _extract_url(req.sample_curl)
        or explicit_endpoint
        or docs_analysis.get("endpoint_url", "")
    )
    if not raw_url:
    ...
    params = _merge_params(
        _extract_params_from_url(parsed), docs_analysis.get("params", [])
    )

    explicit_auth_type = (req.auth_type or "").strip()
    auth_type = (
        docs_analysis.get("auth_type", "none")
        if explicit_auth_type in {"", "none"}
        else explicit_auth_type
    )
    ...
    if auth_config.get("auth_type") == "APIKey" and auth_config.get("in") == "query":
        ...
        params = [
            param
            for param in params
            if not (param.location == "query" and param.name == key_name)
        ]
    elif req.api_key and auth_type == "none":
        warnings.append(
            "已提供 API Key，但认证方式仍是无认证；如需自动携带，请改成 API Key 查询参数或请求头。"
        )

    description = (
        req.description.strip()
        or docs_analysis.get("description", "")
        or f"调用 {parsed.netloc}{path}"
    )
    simple_api_config = SimpleApiConfig(
        base_url=base_url,
        path=path,
...
def _build_remote_api_assist_resp_from_agent_draft(
    ...
    model = (
        agent_draft
        if isinstance(agent_draft, AgenticRemoteApiDraft)
        else AgenticRemoteApiDraft(**agent_draft)
    )
    raw_url = _extract_url(req.sample_curl) or model.endpoint_url or (
        req.endpoint_url or ""
    ).strip()
    if _looks_like_docs_endpoint(raw_url, _resolve_docs_urls(req.docs_url, req.docs_urls)):
        raise ValueError(
            "Agent 返回的是文档或 OpenAPI/Swagger 规格文件地址，不是可调用的业务接口地址。"
            "请补充具体接口页、OpenAPI servers/paths 信息，或提供 curl 示例。"
        )
    parsed = urlparse(raw_url)
    if not parsed.scheme or not parsed.netloc:
    ...
    if _is_missing_or_placeholder_description(agent_description, parsed):
        agent_description = ""
    if (
        not agent_description
        and display_name
        and operation_summary
        and operation_summary.lower() != display_name.lower()
    ):
    ...
    if _is_missing_or_placeholder_description(description, parsed):
        raise ValueError(
            "Agent 没有从文档生成有效描述。请补充更具体的接口文档、OpenAPI/Swagger 链接，或 curl 示例。"
        )
    operation_id = _safe_operation_id(
        display_name
        or operation_summary
        or f"{method.lower()}_{(parsed.path or '/').strip('/').replace('/', '_') or 'root'}"
    )
    auth_config = normalize_remote_api_auth_config(
        {
    ...
    if req.api_key and model.auth_type == "none":
        raise ValueError(
            "已拿到 API Key，但 Agent 没有识别出它应该放在 Query、Header 还是 Bearer 中。"
            "请补一条 curl 示例，或在高级设置里手动选择认证方式。"
        )
    if (
        req.api_key
        and model.auth_type in {"api_key_query", "api_key_header"}
        and not model.api_key_name.strip()
    ):
        raise ValueError(
            "已识别 API Key 认证，但没有识别出字段名。请补充包含 access_key/api_key/x-api-key 的文档或 curl 示例。"
        )
    ...
    required_param_names = [
        param.name
        for param in params
        if param.required and param.location in {"query", "path", "header"}
    ]
    probe_args = _normalize_probe_args(model.probe_args, params)
    if required_param_names:
        missing_probe_args = [
            name for name in required_param_names if name not in probe_args
        ]
        if missing_probe_args:
            warnings.append(
                "Agent 已识别出必填业务参数："
                f"{', '.join(missing_probe_args)}。"
                "未找到安全测试值时，连通性检测会返回缺少测试参数，不会发起真实请求。"
            )
        else:
            warnings.append(
                "Agent 已为必填业务参数生成测试值："
                f"{', '.join(required_param_names)}。"
                "连通性检测会使用这些示例值发起真实请求。"
            )
...
def _resolve_docs_urls(docs_url: str, docs_urls: list[str]) -> list[str]:
    candidates = [docs_url, *docs_urls]
    resolved: list[str] = []
    seen: set[str] = set()
...
def _looks_like_docs_endpoint(endpoint_url: str, docs_urls: list[str]) -> bool:
    ...
    if any(
        token in lowered
        for token in ["/documentation", "/docs", "/swagger", "/openapi", "postman"]
    ):
        return True
...
def _pick_value(
    primary: Optional[str], secondary: Optional[str], fallback: str = ""
) -> str:
...
def _merge_params(
    primary: list[SimpleApiParamConfig],
    secondary: list[SimpleApiParamConfig],
) -> list[SimpleApiParamConfig]:
...
def _normalize_probe_args(
    probe_args: dict[str, Any] | None, params: list[SimpleApiParamConfig]
) -> dict[str, Any]:
...
def _analyze_docs_bundle(docs_urls: list[str]) -> dict[str, Any]:
    ...
    if endpoint and endpoint.startswith("http://") and "https://" not in source_text:
        warnings.append(
            "文档示例使用的是 HTTP；如果服务支持 HTTPS，建议优先改成 HTTPS。"
        )
...
def _collect_docs_text_bundle(docs_urls: list[str]) -> dict[str, Any]:
    ...
    while queue and len(visited) < MAX_DOC_SOURCE_COUNT:
        ...
        for related_url in _extract_related_doc_links(html_text, current_url):
            if (
                related_url not in visited
                and related_url not in queue
                and len(visited) + len(queue) < MAX_DOC_SOURCE_COUNT
            ):
...
def _fetch_docs_page(docs_url: str) -> tuple[str, str]:
    ...
    try:
        ...
        raise RuntimeError(
            f"{request_error}; browser render fallback failed: {browser_error}"
        ) from browser_error
...
def _normalize_docs_html(raw_html: str) -> tuple[str, str]:
    ...
    normalized_text = re.sub(
        r"<script[\s\S]*?</script>", " ", normalized_html, flags=re.IGNORECASE
    )
    normalized_text = re.sub(
        r"<style[\s\S]*?</style>", " ", normalized_text, flags=re.IGNORECASE
    )
...
def _should_render_docs_with_browser(html_text: str, plain_text: str) -> bool:
    ...
    return any(token in lowered for token in js_app_tokens) and "access_key" not in (
        plain_text or ""
    ).lower()
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                f"{playwright_error}; selenium fallback failed: {selenium_error}"
            ) from selenium_error
...
def _extract_related_doc_links(html_text: str, current_url: str) -> list[str]:
    ...
    current_root = (
        ...
        else parsed_current.path.rsplit("/", 1)[0]
    )
    links: list[str] = []
    seen: set[str] = set()
...
def _score_doc_url(value: str) -> tuple[int, int]:
    ...
    if any(
        token in lowered
        for token in ["documentation", "getting-started", "quickstart", "docs/"]
    ):
        score -= 10
...
def _extract_doc_params(text: str) -> list[SimpleApiParamConfig]:
    ...
    for key, description in re.findall(
        r'"([a-z_][a-z0-9_]*)","([^"]{5,160})"', text, flags=re.IGNORECASE
    ):
    ...
    for key in candidates:
        ...
        if (
            not normalized
            or lowered in seen
            or lowered in IGNORED_DOC_PARAM_NAMES
            or lowered == "access_key"
        ):
            continue
        params.append(
            SimpleApiParamConfig(
...
def _extract_doc_display_name(text: str, endpoint_url: str) -> str:
    title_match = re.search(
        r"\b([A-Z][A-Za-z0-9_-]{2,})\b(?: API| Quickstart Guide| Documentation)", text
    )
    if title_match:
    ...
    host_match = re.search(
        r"Base URL[:\s]+https?://(?:api\.)?([A-Za-z0-9-]+)",
        text,
        re.IGNORECASE,
    )
    if host_match:
