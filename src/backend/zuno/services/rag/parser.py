...
class DocParser:
            ...
            while start < len(content):
                ...
                limited_chunks.append(
                    ChunkModel(
                        ...
                        source_chunk_id=f"{chunk.source_chunk_id}_part_{part_index}"[:128],
                    )
                )
                part_index += 1
            ...
            if image_mode == "text_only":
                ...
                chunks = [
                    ChunkModel(
                        ...
                        source_chunk_id=f"{file_id}_image_0",
                    )
                ]
            else:
                chunks = [
    ...
    async def generate_summary(cls, chunk: ChunkModel, semaphore):
        ...
        async with semaphore:
            prompt = f"""
                浣犳槸涓€涓笓涓氱殑鎽樿鐢熸垚鍔╂墜锛岃鏍规嵁浠ヤ笅瑕佹眰涓烘枃鏈敓鎴愪竴娈垫憳瑕侊細
                ## 闇€瑕佹€荤粨鐨勬枃鏈細
                {chunk.content}
                ## 瑕佹眰锛?                1. 鎽樿瀛楁暟鎺у埗鍦?100 瀛楀乏鍙炽€?                2. 鎽樿涓粎鍖呭惈鏂囧瓧鍜屽瓧姣嶏紝涓嶅緱鍑虹幇閾炬帴鎴栧叾浠栫壒娈婄鍙枫€?                3. 鍙緭鍑烘憳瑕侀儴鍒嗭紝涓嶅噯杈撳嚭 鈥滀互涓嬫槸鏂囨湰鐨勬憳瑕佲€?绛夊瓧鏍枫€?            """
