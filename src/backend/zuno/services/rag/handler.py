...
class RagHandler:
    ...
    async def retrieve_ranked_documents_with_metadata(
        ...
        orchestrator = RetrievalOrchestrator(
            rag_retriever=RagRetrieverAdapter(),
            keyword_retriever=BM25RetrieverAdapter(),
            graph_retriever=GraphRetrieverAdapter(),
            query_expander=QueryExpanderAdapter(),
            planner=RetrievalPlanner(enable_keyword_recall=app_settings.rag.enable_elasticsearch),
    ...
    async def delete_documents_es_milvus(cls, file_id, knowledge_id):
        await cls.delete_documents_by_file(file_id, knowledge_id)

    @classmethod
    async def delete_documents_by_file(cls, file_id, knowledge_id):
        if app_settings.rag.enable_elasticsearch:
        ...
        if Neo4jClient.is_enabled():
            ...
            await graph_client.delete_by_knowledge_file(file_id, knowledge_id)
