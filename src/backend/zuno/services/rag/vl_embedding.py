...
def _get_vl_config(config_override=None):
    return _normalize_model_config(config_override) or ModelManager.get_model_config("vl_embedding", "VL Embedding model")
...
async def _request_vl_embedding(contents: list[dict], config_override=None) -> list[list[float]]:
    ...
    payload = {
        ...
        "parameters": {"dimension": VL_EMBEDDING_DIMENSION},
    }
    headers = {
