...
`src/backend/zuno/api/dto/` 是 API DTO / Pydantic schema 的当前物理 owner。旧 public import path `zuno.schema.*` 仅通过 `platform/compatibility/legacy_aliases.py` 作为迁移兼容面映射到这里；它是兼容 alias，不是仍存在的 root-level schema 目录。
