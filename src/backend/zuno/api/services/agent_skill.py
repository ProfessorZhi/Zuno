...
class AgentSkillService:
    ...
    def _load_host_skill(cls, skill_dir: Path) -> SimpleNamespace | None:
        ...
        skill_name = str(skill_json.get("name") or frontmatter.get("name") or folder_name).strip()
        description = (
            str(skill_json.get("description") or frontmatter.get("description") or "").strip()
            or f"Host skill loaded from {skill_dir}"
        )
...
__all__ = ["AgentSkillService", "default_agent_skill_folder"]
