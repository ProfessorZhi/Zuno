...
from zuno.knowledge.agentic import AgenticGraphRAGRuntime
...
from zuno.platform.services.workspace.single_controller_runtime import configure_workspace_product_composition
...
class ProductRuntimeMechanics:
    """Shared execution engine used by the Product application services.

    This class owns runtime mechanics and durable state transitions. HTTP
    routes do not depend on it directly; they use the narrower application
    ...
    # These stores are intentionally unbound at import time. Production state
    # must be injected by the server composition root; tests must provide an
    # explicit store through the test-only configuration methods below.
    _agent_run_store: SQLiteAgentRunStore | None = None
    _knowledge_index_runtime = KnowledgeIndexRuntime()
    _agentic_retrieval_runtime = AgenticGraphRAGRuntime(index_runtime=_knowledge_index_runtime)
    ...
    def configure_agent_run_store_for_tests(cls, store: SQLiteAgentRunStore) -> None:
        """Bind the explicit Agent Run store for direct application tests."""
    ...
    def reset_runtime_state_for_tests(cls) -> None:
        ...
        cls._agentic_retrieval_runtime = AgenticGraphRAGRuntime(index_runtime=cls._knowledge_index_runtime)
        cls._trace_spans = {}
        cls._release_evals = {}
        cls._trace_replays = {}
        ...
        # Tests must never create the application composition implicitly. The
        # composition is wired explicitly at startup and reset clears any
        # previous binding so tests stay isolated.
    ...
    def register_file(
        ...
        *,
        ...
        if cls._package_a_production_runtime is None and (
            cls._durable_ingestion_store is None or cls._source_object_store is None
        ):
            raise HTTPException(
                status_code=503,
                detail="Product ingestion durable owner is not configured",
            )
    ...
    def create_ingest_job(
        ...
        *,
        ...
        if cls._durable_ingestion_store is None or cls._source_object_store is None:
            raise HTTPException(
                status_code=503,
                detail="Product ingestion durable owner is not configured",
            )
    ...
    def create_task(
        ...
        *,
        ...
        # A missing or empty workspace_id fails closed before any model or
        # tool execution. Product context comes from the authenticated request.
        if not workspace_id:
    ...
    def approve_task(
        ...
        *,
        ...
        cls._validate_agent_run_approval_binding(
            task_id=task_id,
        ...
        runtime_snapshot = cls._agent_run_service().resume(
            task_id=task_id,
        ...
        task.status = (
            "completed"
            if runtime_snapshot.finalization_status == "finalized"
    ...
    def cancel_task(cls, *, task_id: str, reason: str | None) -> dict:
        ...
        try:
            cls._agent_run_service().cancel(task_id=task_id, reason=normalized_reason)
    ...
    @classmethod
    def _require_agent_run_store(cls) -> SQLiteAgentRunStore:
        if cls._agent_run_store is None:
            raise BlockedConfiguration(
                "BLOCKED_CONFIGURATION: Agent Run store is not bound; "
                "an explicit durable store is required"
            )
        return cls._agent_run_store

    @classmethod
    def _agent_run_service(cls) -> AgentRuntimeService:
        from zuno.agent.runtime import RuntimeDependencyFactory

        assembly = RuntimeDependencyFactory.for_workspace_task(
            store=cls._require_agent_run_store(),
            knowledge_index_runtime=cls._knowledge_index_runtime,
        )
        return AgentRuntimeService(store=assembly.store, dependencies=assembly.dependencies)

    @classmethod
    def _validate_agent_run_approval_binding(
        cls,
        *,
        task_id: str,
        ...
        for name, supplied in (
            ...
            if supplied is None:
                continue
            expected_value = expected.get(name) or (
                interrupt.required_approval if name == "required_approval" else None
            )
        ...
        if mismatched:
            raise HTTPException(
                status_code=409,
                detail=(
                    "Approval binding mismatch; fetch the current Agent Run "
                    f"interrupt before retrying: {', '.join(mismatched)}"
                ),
            )

    @classmethod
    def _start_agent_run_for_task(
        cls,
        *,
        task: WorkspaceTaskContract,
        simple_task: WorkSpaceSimpleTask,
        ...
        # The runtime rejects empty or fabricated tenant identity before any
        # model or tool is invoked. Identity comes from authenticated server
        # context, never from a request-body fallback.
        ...
        request = RuntimeStartRequest(
            ...
            goal=goal,
            reflection_decision=("use_tool" if approval_required else None),
        )
        for runtime_event in cls._agent_run_service().stream(request):
    ...
    def _document_from_file(*, file: UploadedFileContract, content: str) -> CanonicalDocumentIR:
        return CanonicalDocumentIR(
            metadata=DocumentMetadata(
                ...
                parser_version="agentic-retrieval-v1",
                acl_scope=file.policy_scope,
            ),
            blocks=[
                DocumentBlock(
    ...
    def get_task_snapshot(cls, task_id: str, *, principal_id: str = "") -> dict:
        ...
        agent_run_snapshot = cls._agent_run_service().get_snapshot(task_id)
        ...
        return {
            ...
            "lifecycle": cls._task_lifecycle_snapshot(
                task=task,
                runtime_snapshot=agent_run_snapshot,
            ).model_dump(),
            "agent_run": agent_run_snapshot.model_dump(mode="json") if agent_run_snapshot is not None else None,
            "observability": {
    ...
    def _artifact_download_url(artifact_id: str) -> str:
        return f"/api/v1/product/artifacts/{artifact_id}/download"
        ...
        try:
            ...
            return CapabilityPlanningRuntime(
            ...
            ).select(
                {
                    ...
                    "workspace_id": task.workspace_id,
                    "user_id": login_user.user_id,
                    # No synthetic user:* tenant. The capability-plan layer
                    # receives the server-owned tenant from authenticated
                    # context.
                    "tenant_id": (getattr(login_user, "tenant_id", "") or "").strip(),
                    "user_goal": task.goal,
                    "available_capability_ids": tuple(simple_task.plugins),
    ...
    def _record_product_runtime_request(
        ...
        *,
        ...
        # Resolve the trusted tenant before touching the submitter. The
        # server-owned tenant must come from authenticated context, never from
        # the request body or a user-derived fallback. Missing, synthetic or
        # default tenant identity fails closed.
        ...
        try:
            result = submitter(
                ...
                client_request_id=f"product:{task.task_id}:{request_hash}",
                runtime_request_ref=f"product-runtime-request:{task.task_id}:{request_hash}",
                raw_intent_ref=f"product-intent:{task.task_id}:{request_hash}",
                payload=payload,
    ...
    def _record_release_eval(
        ...
        *,
        ...
        baseline = ReleaseEvalBaseline(
            dataset_version="product-runtime-v1",
    ...
    def _fail_task(
        ...
        *,
        ...
        agent_run_store = cls._agent_run_store
        if agent_run_store is not None and agent_run_store.has_task(task_id):
            agent_run_store.mark_failure(
                task_id,
                {
...
# Product run execution is intentionally not bound at module import time.
# The current Product API submits durable commands; the test-only run owner
# requires explicit stores and fails closed when they are absent.
