...
async def upload_file(
    knowledge_id: str = Body(..., description="鐭ヨ瘑搴?ID"),
    file_url: str = Body(..., description="鏂囦欢涓婁紶鍚庤繑鍥炵殑 URL"),
    login_user: UserPayload = Depends(get_login_user),
):
    try:
        prepared_upload = KnowledgeFileService.prepare_uploaded_file(file_url)
        result = await KnowledgeFileService.create_knowledge_file(
            file_name=prepared_upload["file_name"],
            file_path=prepared_upload["local_file_path"],
...
__all__ = [
    ...
    "get_knowledge_file_task",
    "list_knowledge_file_tasks",
    "router",
