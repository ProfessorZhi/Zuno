    ...
    async def test_connectivity(self) -> dict[str, Any]:
        ...
        if not Path(executable).is_absolute():
            ...
            if not resolved_executable:
                raise ValueError(f"CLI executable not found: {executable}")
        ...
        if not healthcheck_parts:
            return {
                ...
                "details": [
                    f"tool_dir: {tool_dir}",
                    f"cwd: {cwd}",
                    f"command: {executable_display}",
                ],
                "warnings": ["Healthcheck command is not configured; only directory and executable validation was performed."],
                "executed": False,
        ...
        return {
            "ok": True,
            "runtime_type": "cli",
            "summary": "CLI healthcheck succeeded",
            "details": details,
            "warnings": [],
            "executed": True,
            "command": " ".join(healthcheck_parts),
