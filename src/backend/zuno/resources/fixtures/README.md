...
`src/backend/zuno/resources/fixtures/` 当前保存 runtime 或测试可能消费的内置资源。它是 app-resource，存在和 `tests/fixtures`、`examples/` 或 runtime default resource 的边界混合风险。

## Target role

目标状态下，fixtures 必须按用途拆清：测试专用进入 `tests/`，示例输入进入 `examples/`，runtime 默认资源才留在后端 app-resource 中。本轮先把后端内置资源收敛到 `resources/fixtures/`，后续再按用途细拆。
...
- 禁止恢复顶层 `src/backend/zuno/fixtures/`。
