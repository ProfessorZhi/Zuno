...
class ToolConnectivityService:
    ...
    async def test_system_tool(tool_name: str) -> ToolConnectivityResp:
        from zuno.api.v1.config import SYSTEM_TOOL_CONFIG_META, _load_runtime_config, _normalize_email_accounts
        ...
        return ToolConnectivityResp(
            ok=False,
            runtime_type="cli",
            summary=f"Unsupported system tool kind: {tool_kind}",
            warnings=[tool_kind],
        )
    ...
    async def _test_system_remote_api(tool_name: str, meta: dict[str, Any], values: dict[str, Any]) -> ToolConnectivityResp:
        ...
        if not endpoint:
            return ToolConnectivityResp(
                ok=False,
                runtime_type="remote_api",
                summary="系统 API 工具缺少 endpoint，未执行真实请求",
                warnings=["缺少 endpoint"],
            )
    ...
    async def _test_system_smtp(config_data: dict[str, Any], normalize_accounts) -> ToolConnectivityResp:
        accounts = [
            item
            for item in normalize_accounts(config_data)
        ...
        if not accounts:
            return ToolConnectivityResp(
                ok=False,
                runtime_type="cli",
                summary="邮件工具缺少可用邮箱槽位，未执行 SMTP 检测",
                warnings=["请先配置邮箱槽位"],
            )
        ...
        try:
            ...
            return ToolConnectivityResp(
                ok=False,
                runtime_type="cli",
                summary="SMTP 登录/握手检测失败",
                details=[f"slot: {account['slot_name']}", str(err)],
                warnings=[str(err)],
                executed=True,
            )
        ...
        return ToolConnectivityResp(
            ok=True,
            runtime_type="cli",
            summary="SMTP 登录/握手检测成功",
            details=[f"slot: {account['slot_name']}", account["smtp_host"]],
            executed=True,
        )
    ...
    async def _test_system_public_source(tool_name: str) -> ToolConnectivityResp:
        ...
        return ToolConnectivityResp(
            ok=True,
            runtime_type="cli",
            summary="公开数据源 smoke test 成功",
            details=["arxiv returned at least one result"],
            executed=True,
        )
...
__all__ = ["ToolConnectivityService"]
