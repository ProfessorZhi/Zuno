...
async def upload_user_avatars_storage():
    if not _is_storage_endpoint_reachable():
        logger.warning("Skip avatar upload because storage endpoint is not reachable")
        return

    existing_files = await asyncio.to_thread(storage_client.list_files_in_folder, "icons/user")
    if not existing_files:
        ...
        for avatar_url in user_avatars["avatars"]:
            ...
            await asyncio.to_thread(storage_client.upload_file, object_name, image_data)
...
def _is_storage_endpoint_reachable(timeout_seconds: float = 1.0) -> bool:
    storage = app_settings.storage
    if storage is None:
        return False

    endpoint = ""
    if storage.mode == "minio" and storage.minio:
        endpoint = storage.minio.endpoint or ""
    elif storage.mode == "oss" and storage.oss:
        endpoint = storage.oss.endpoint or ""
    ...
    try:
        port = int(raw_port) if raw_port else (9000 if storage.mode == "minio" else 443)
    except ValueError:
        port = 9000 if storage.mode == "minio" else 443
