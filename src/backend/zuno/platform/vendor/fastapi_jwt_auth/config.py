...
class LoadConfig(BaseModel):
    ...
    @classmethod
    def validate_denylist_token_checks(cls, values):
        if values is None:
            return values
        for value in values:
    ...
    @classmethod
    def validate_token_location(cls, values):
        if values is None:
            return values
        for value in values:
    ...
    @classmethod
    def validate_csrf_methods(cls, values):
        if values is None:
            return values
        normalized = []
        for value in values:
            upper = value.upper()
            if upper not in {"GET", "HEAD", "POST", "PUT", "DELETE", "PATCH"}:
