    ...
    def convert_single_mcp(server_info):
        if isinstance(server_info, dict):
            if server_info.get("type") == "sse":
                ...
                env = server_info.get("env") or {}
                env_passthrough = server_info.get("env_passthrough") or []
                if isinstance(env_passthrough, list):
                    for key in env_passthrough:
                        if key and key not in env and key in os.environ:
...
def function_to_args_schema(func) -> dict:
    ...
    for param in signature.parameters.values():
        try:
            ...
            raise KeyError(f"Unknown schema annotation {param.annotation} for parameter {param.name}: {str(e)}")
