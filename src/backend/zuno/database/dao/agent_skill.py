...
class AgentSkillDao:
    ...
    async def get_agent_skills(cls, user_id):
        async with async_session_getter() as session:
            statement = select(AgentSkill).where(AgentSkill.user_id == user_id)
    ...
    async def get_agent_skill_by_id(cls, agent_skill_id) -> AgentSkill:
        async with async_session_getter() as session:
            statement = select(AgentSkill).where(AgentSkill.id == agent_skill_id)
            result = await session.exec(statement)
            return result.first()

    @classmethod
    async def get_agent_skills_by_ids(cls, agent_skill_ids: List[str]) -> List[AgentSkill]:
        if not agent_skill_ids:
            return []

        async with async_session_getter() as session:
            statement = select(AgentSkill).where(AgentSkill.id.in_(agent_skill_ids))
            result = await session.exec(statement)
            return result.all()
