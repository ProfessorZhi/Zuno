...
from agentchat.schema.mcp_user_config import MCPUserConfigCreateRequest, MCPUserConfigUpdateRequest, MCPUserConfigTestRequest
...
@router.post('/mcp_user_config/test', response_model=UnifiedResponseModel)
async def test_mcp_user_config(
    *,
    config_request: MCPUserConfigTestRequest,
    login_user: UserPayload = Depends(get_login_user)
):
    try:
        await MCPService.verify_user_permission(config_request.server_id, login_user.user_id, action="test")
        result = await MCPService.test_user_config(config_request.server_id, login_user.user_id)
